...
if (fs.existsSync("./code_dictionary.json")) {
  ...
  const jsonifySets = (_key, value) =>
    value instanceof Set ? [...value] : value;
  fs.writeFileSync(
    "./code_inventory.json",
    JSON.stringify(codeInventory, jsonifySets, 2),
  );
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(codeDictionary, null, 2),
  );
...
function walkObject(object, codeFunction) {
  if (Array.isArray(object)) {
    ...
    if (object.system && object.code && object.display) {
      if (
        SYSTEMS_TO_SKIP.includes(object.system) ||
        PLACEHOLDER_CODES.includes(object.code.toString())
      ) {
        return;
      }
      codeFunction(object);
...
function buildDictionary() {
  for (const codeObj of allCodes) {
    ...
    if (!systemUri) {
      systemUri = system;
      console.log(
        `Unexpected code system ${system} may not be supported by tx.fhir.org`,
      );
    }
    ...
    if (displayParam) {
      ...
      const bestDisplay = selectBestCode(
        system,
        codeInventory[system][code],
        aPreferredDisplay,
        currentDisplay,
      );
...
function selectBestCode(system, inventory, aPreferredDisplay, currentDisplay) {
  if (system === "SNOMED-CT" && !aPreferredDisplay.endsWith(")")) {
    ...
    for (const option of inventory) {
      ...
      const regexSafeDisplay = aPreferredDisplay
        .replaceAll("+", "\\+")
        .replaceAll("(", "\\(")
        .replaceAll(")", "\\)");
      if (
        option.match(new RegExp(`^${regexSafeDisplay} \\([a-z\\+/ ]+\\)$`, "i"))
      ) {
        return option;
      }
    }
...
function checkAndUpdateCode(codeObj) {
  ...
  if (system === "SNOMED-CT") {
    const regexSafeDisplay = display
      .replaceAll("+", "\\+")
      .replaceAll("(", "\\(")
      .replaceAll(")", "\\)");
    if (
      bestDisplay.match(
        new RegExp(`^${regexSafeDisplay} \\([a-z\\+/ ]+\\)$`, "i"),
      )
    ) {
...
function handleErrorOrExit(requestUrl, res) {
  ...
  if (messageParam) {
    ...
    if (
      message.startsWith("Unable to find code ") ||
