...
fetchAllCOVIDData().then((allData) => {
  ageCategories.forEach((category) => {
    const categoryData = allData.filter(
      (row) => row.demographic_category == category,
    );
    for (i = 0; i < categoryData.length; i++) {
      ...
      if (i == 0) {
        ...
        doseCount =
          categoryData[i].administered_dose1 -
          categoryData[i - 1].administered_dose1;
      }
  ...
  fs.writeFileSync(
    "../resources/covid19/covid_first_shot_percentage_by_age.json",
