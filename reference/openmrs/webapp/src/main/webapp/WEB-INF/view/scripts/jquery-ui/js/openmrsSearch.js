...
function OpenmrsSearch(
	...
	if (!opts.showIncludeVoided) opts.showIncludeVoided = showIncludeVoided;
	if (!opts.selectionHandler) opts.selectionHandler = selectionHandler;
	if (!opts.searchHandler) opts.searchHandler = searchHandler;
	if (!opts.fieldsAndHeaders) opts.fieldsAndHeaders = fieldsAndHeaders;
...
(function ($j) {
	...
	if (!Number(BATCH_SIZE)) BATCH_SIZE = 200;
	var ajaxTimer = null;
	var buffer = null;
	var inSerialMode = Boolean(gp.searchRunInSerialMode);
	var MAXIMUM_NUMBER_OF_RESULTS = gp.maximumResults;
	if (!Number(MAXIMUM_NUMBER_OF_RESULTS)) MAXIMUM_NUMBER_OF_RESULTS = 2000;
	$j.widget("ui.openmrsSearch", {
		...
		_create: function () {
			...
			minCharErrorObj.html(
				omsgs.minCharRequired.replace("_REQUIRED_NUMBER_", o.minLength),
			);
			...
			if (o.displayLength < 3) o.displayLength = 3;

			// If need search button
			if (showSearchButton) {
				input.after(
					"<input type='button' id='searchButton' name='searchButton' value='" +
						omsgs.searchLabel +
						"' />",
				);
				$j("#searchButton").click(function () {
					if (
						$j.trim(input.val()) != "" ||
						self.options.doSearchWhenEmpty
					) {
			...
			if (o.showIncludeVoided) {
				...
				tmp.after(
					"<label for='includeVoided'>" +
						o.includeVoidedLabel +
						"</label>",
				);
				tmp.show();
				...
				if (userProperties.showRetired) tmp.prop("checked", true);
			}

			if (o.showIncludeVerbose) {
				var tmp = div.find("#includeVerbose");
				tmp.after(
					"<label for='includeVerbose'>" +
						o.includeVerboseLabel +
						"</label>",
				);
				tmp.show();
				...
				if (userProperties.showVerbose) tmp.prop("checked", true);
			}
			...
			input.keyup(function (event) {
				...
				if (!inSerialMode && ajaxTimer) window.clearInterval(ajaxTimer);

				var searchDelay = SEARCH_DELAY;
				if (
					text.length < o.minLength &&
					!self.options.doSearchWhenEmpty
				) {
			...
			for (var x in inputs) {
				...
				if (
					...
					$j(inputField).prop("disabled") == false &&
					$j(inputField).is(":visible") &&
					$j(inputField).css("visibility") != "hidden"
				)
					numberOfTextInputs++;
			}

			if (numberOfTextInputs == 1) input.focus();

			if (self.options.initialData)
				self._results = self.options.initialData;
			else div.find(".openmrsSearchDiv").hide();
			...
			if (self.options.searchPlaceholder) {
				...
				var textShown = self.options.searchPlaceholder
					.toString()
					.replace(/(\r\n|\n|\r)/gm, "");
			...
			this._table = table.dataTable({
				...
				fnDrawCallback: function (oSettings) {
					...
					if (
						!self.options.showIncludeVerbose ||
						!self._table ||
						self._table.fnGetNodes().length == 0
					)
						return;
					pageRowCount =
					...
					for (
						...
						if (
							self.options.showIncludeVerbose &&
							self.options.verboseHandler
						) {
							rowData = self._results[i];
							verboseText = self.options.verboseHandler(
								i,
								rowData,
							);
							...
							$j(verboseRow).hover(
								function () {
									...
									if (self.curRowSelection != null) {
										currNode =
											self._table.fnGetNodes()[
												self.curRowSelection
											];
										self._unHighlightRow(currNode);
										self._unHighlightVerboseRow(
											currNode.nextSibling,
										);
									}
									self.hoverRowSelection = i;
									$j(this.previousSibling).addClass(
										"row_highlight",
									);
								},
								function () {
									if (self.curRowSelection != null) {
										...
										$j(currNode.nextSibling).addClass(
											"row_highlight",
										);
									}
									...
									if (
										self.curRowSelection != null &&
										self._table.fnGetPosition(dataRow) ==
											self.curRowSelection
									)
										return;
							...
							if (
								rowData &&
								(rowData.voided || rowData.retired)
							) {
								$j(verboseRow)
									.children()
									.each(function () {
							...
							if (self.options.selectionHandler) {
								$j(verboseRow)
									.unbind("click")
									.bind("click", function () {
										rowIndex = self._table.fnGetPosition(
											this.previousSibling,
										);
										//Onclick handlers should work on the verbose row too
										self._doSelected(
											rowIndex,
											self._results[rowIndex],
										);
									});
							}
						}
				...
				fnRowCallback: function (
					...
					$j(nRow).hover(
						function () {
							...
							if (
								self.options.showIncludeVerbose &&
								$j(verboseCheckBox).prop("checked")
							)
						...
						function () {
							if (self.curRowSelection != null) {
								...
								if (self.options.showIncludeVerbose)
									$j(currentNode.nextSibling).addClass(
										"row_highlight",
									);
							}
							self.hoverRowSelection = null;
							if (
								self.curRowSelection != null &&
								self._table.fnGetPosition(this) ==
									self.curRowSelection
							)
								return;
							if (
								...
								$j(this.nextSibling).removeClass(
									"row_highlight",
								);
						},
					);
			...
			var selectElement = document
				.getElementById("openmrsSearchTable_length")
			...
			if (self.options.initialData) {
				...
				if (
					initialRowCount %
						self._table.fnSettings()._iDisplayLength ==
					0
				)
					self._table.numberOfPages =
						initialRowCount /
						self._table.fnSettings()._iDisplayLength;
				else
					self._table.numberOfPages =
			...
			inputNodeDOM.focus();
			if (inputNodeDOM) {
				var searchPhraseLength = self.options.searchPhrase
					? self.options.searchPhrase.length
					: 0;
				//setSelectionRange is for setting the cursor at the end of the search text
				if (inputNodeDOM.setSelectionRange) {
					inputNodeDOM.setSelectionRange(
						searchPhraseLength,
						searchPhraseLength,
					);
				}
		...
		_makeColumns: function () {
			...
			columData = $j.map(fieldsAndHeaders, function (c) {
				...
				if (
					self.options.columnWidths &&
					self.options.columnWidths[columnIndex]
				)
					width = self.options.columnWidths[columnIndex];
				if (
					self.options.columnRenderers &&
					self.options.columnRenderers[columnIndex]
				)
					fnRenderer = self.options.columnRenderers[columnIndex];
				if (
					self.options.columnVisibility &&
					self.options.columnVisibility[columnIndex] == false
				)
					visible = false;
			...
			if (self.options.attributes) {
				$j.each(self.options.attributes, function (index, a) {
					...
					if (a.columnWidth) attribColWidth = a.columnWidth;
					if (a.columnRenderer)
						attribColFnRenderer = a.columnRenderer;

					columData.push({
						sTitle: a.header,
						sWidth: attribColWidth,
						fnRender: attribColFnRenderer,
						bVisible: attribColVisibility,
					});
				});
			}
		...
		_doSearch: function (text) {
			if (this.options.searchHandler) {
				var tmpIncludeVoided =
					this.options.showIncludeVoided && checkBox.prop("checked");
				...
				this.options.searchHandler(
					text,
					this._handleResults(text, storedCallCount),
					true,
					{
						includeVoided: tmpIncludeVoided,
						start: 0,
		...
		_handleResults: function (searchText, curCallCount) {
			...
			return function (results) {
				...
				if (matchCount > self._table.fnSettings()._iDisplayLength) {
					...
					if (matchCount == self._results.length) {
						...
						if (
							...
							self._table.numberOfPages =
								Math.floor(
									self._results.length /
										self._table.fnSettings()
											._iDisplayLength,
								) + 1;

						$j("#pageInfo").append(
							" - " +
								omsgs.pagesWithPlaceHolder.replace(
									"_NUMBER_OF_PAGES_",
									self._table.numberOfPages,
								),
						);
					...
					loadingMsgObj.html(
						omsgs.loadingWithArgument.replace(
							"_NUMBER_OF_PAGES_",
							matchCount,
						),
					);
		...
		_fetchMoreResults: function (
			...
			this.options.searchHandler(
				...
				{
					includeVoided:
						this.options.showIncludeVoided &&
		...
		_doKeyDown: function () {
			...
			if (this.hoverRowSelection != null) return;

			var prevRow = this.curRowSelection;
			//if we are on the last page, and the last row is highlighted, do nothing
			if (
				this._getCurrVisiblePage() == this._table.numberOfPages &&
				prevRow >= this._results.length - 1 &&
			...
			if (this._isHighlightedRowOnVisiblePage()) {
				...
				if (
					...
					this.curRowSelection %
						this._table.fnSettings()._iDisplayLength ==
						0
				) {
		...
		_doKeyUp: function () {
			...
			if (
				this._table.fnSettings()._iDisplayStart == 0 &&
				this.curRowSelection == 0
			)
				return;
			...
			if (this._isHighlightedRowOnVisiblePage()) {
				...
				if (this._getCurrVisiblePage() < this._table.numberOfPages) {
					this.curRowSelection =
						this._table.fnSettings()._iDisplayStart +
						this._table.fnSettings()._iDisplayLength -
						1;
				} else {
		...
		_doPageUp: function () {
			this._table.fnPageChange("previous");
			if (this._isHighlightedRowOnVisiblePage()) return;
		...
		_doKeyEnter: function () {
			...
			if (selectedRowIndex != null) {
				this._doSelected(
					selectedRowIndex,
					this._results[selectedRowIndex],
				);
			} else if (showSearchButton) {
				if (
					$j.trim($j("#inputNode").val()) != "" ||
					self.options.doSearchWhenEmpty
				) {
		...
		_unHighlightVerboseRow: function (vRow) {
			if (vRow) {
				...
				if ($j(vRow).hasClass("row_highlight"))
					...
					$j(vRow).css(
						"background-color",
						$j(vRow.previousSibling).css("background-color"),
					);
			}
		},
		...
		_getVisibleColumnCount: function () {
			...
			$j.map(self.options.fieldsAndHeaders, function (c) {
				if (self.options.columnVisibility[columnIndex] == true) count++;
		...
		_updatePageInfo: function (searchText) {
			textToDisplay = omsgs.viewingResultsFor.replace(
				"_SEARCH_TEXT_",
				"'<b>" + searchText + "</b>'",
			);
			if ($j.trim(searchText) == "") textToDisplay = omsgs.viewingAll;
		...
		_addMoreRows: function (
			...
			return function (results) {
				...
				if (inSerialMode || curSubCallCount == self._lastSubCallCount) {
					...
					if (
						!inSerialMode &&
						self._bufferedAjaxCallCounters.length > 0
					) {
						...
						while (true) {
							...
							for (var i in self._bufferedAjaxCallCounters) {
								subCallCounter =
									self._bufferedAjaxCallCounters[i];
								...
								if (
									subCallCounter &&
									subCallCounter == nextSubCallCount &&
									bufferedData
								) {
									...
									for (var j in bufferedData) {
										...
										rowsToInsert.push(
											self._buildRow(bufferedRowData),
										);
							...
							if (!wasNextSubCallInBuffer) break;

							//remove the sub call counter
							self._bufferedAjaxCallCounters.splice(
								foundAtIndex,
								1,
							);
						}
					...
					!inSerialMode &&
					curSubCallCount > self._lastSubCallCount
				) {
				...
				if (actualResultCount >= matchCount) {
					...
					loadingMsgObj.html("");
					$j("#pageInfo").html(
						omsgs.viewingResultsFor.replace(
							"_SEARCH_TEXT_",
							"'<b>" + searchText + "</b>'",
						),
					);
					pageStr = omsgs.pagesWithPlaceHolder.replace(
						"_NUMBER_OF_PAGES_",
						self._table.numberOfPages,
					);
