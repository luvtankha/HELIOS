...
(function (d) {
		...
		stop: function (b) {
			...
			a.element.animate(d.extend(g, h && f ? { top: h, left: f } : {}), {
				...
				step: function () {
					...
					e &&
						e.length &&
						d(e[0]).css({ width: i.width, height: i.height });
					a._updateCache(i);
	...
	d.ui.plugin.add("resizable", "grid", {
		resize: function () {
			...
			var h =
				Math.round((c.width - e.width) / (a.grid[0] || 1)) *
				(a.grid[0] || 1);
			a =
				Math.round((c.height - e.height) / (a.grid[1] || 1)) *
				(a.grid[1] || 1);
			if (/^(se|s|e)$/.test(f)) {
		...
		activate: function (a, c) {
			...
			if (this.hasScroll()) {
				...
				if (d < 0) this.element.attr("scrollTop", b + d);
					...
					d > f &&
						this.element.attr("scrollTop", b + d - f + c.height());
			}
			this.active = c
				.eq(0)
				.children("a")
				...
				if (!this.active || this.last())
					...
					var c = this.active.offset().top,
						...
						b = this.element.children("li").filter(function () {
							var f =
								e(this).offset().top - c - d + e(this).height();
							return f < 10 && f > -10;
						});
		...
		_refreshValue: function () {
			...
			if (this.options.values && this.options.values.length)
				this.handles.each(function (l) {
					f =
						((c.values(l) - c._valueMin()) /
							(c._valueMax() - c._valueMin())) *
						100;
					g[c.orientation === "horizontal" ? "left" : "bottom"] =
						f + "%";
...
(function (d, G) {
	...
	d.extend(K.prototype, {
				...
				var e =
					...
					n = function (p, v, H) {
						...
						for (v = 0; v < p.length; v++)
							if (
								b.substr(s, p[v].length).toLowerCase() ==
								p[v].toLowerCase()
							) {
								s += p[v].length;
								return v + 1;
		...
		_generateHTML: function (a) {
			...
			for (
				var L = this._getDefaultDate(a), I = "", C = 0;
				C < i[0];
				C++
			) {
