(function ($) {
	$.tree = {
		...
		defaults: {
			...
			types: {
				default: {
					...
					max_depth: -1,
					valid_children: "all",
					icon: { image: false, position: false },
				},
			},
			rules: {
				multiple: false,
		...
		drop_mode: function (opts) {
			...
			if (!opts.show) {
				...
				tree_component.drag_drop.drag_node =
					tree_component.drag_drop.drag_help.find("li:eq(0)");
			}
	...
	function tree_component() {
		return {
			...
			init: function (elem, conf) {
				...
				tree_component.inst[this.container.attr("id")] =
					tree_component.inst[this.cntr];
				...
				if (this.settings.languages && this.settings.languages.length) {
					...
					for (
						var ln = 0;
						ln < this.settings.languages.length;
						ln++
					) {
						st = tree_component.add_css(
							id + " ." + this.settings.languages[ln],
						);
						if (st !== false)
							st.style.display =
								this.settings.languages[ln] == this.current_lang
									? ""
									: "none";
					}
				...
				if (this.settings.ui.theme_name !== false) {
					if (this.settings.ui.theme_path === false) {
						$("script").each(function () {
							if (
								this.src.toString().match(/jquery\.tree.*?js$/)
							) {
								_this.settings.ui.theme_path =
									this.src
										.toString()
										.replace(/jquery\.tree.*?js$/, "") +
									"themes/" +
					...
					this.container.addClass(
						"tree-" + this.settings.ui.theme_name,
					);
				}
				var type_icons = "";
				...
				if (tree_component.drag_drop.marker === false)
					tree_component.drag_drop.marker = $("<div>")
			...
			refresh: function (obj) {
				...
				if (this.selected) {
					...
					var tmp = Array();
					if (
						(typeof this.settings.selected).toLowerCase() ==
						"object"
					) {
			...
			reselect: function (is_callback) {
				...
				if (this.opened && this.opened.length) {
					...
					for (
						var j = 0;
						this.opened && j < this.opened.length;
						j++
					) {
			...
			attach_events: function () {
				...
				$("li", this.container.get(0)).live("click", function (event) {
				...
				$("a", this.container.get(0))
					.live("click", function (event) {
						...
						_this.select_branch.apply(_this, [
							event.target,
							event.ctrlKey ||
					...
					.live("dblclick", function (event) {
						...
						_this.callback("ondblclk", [
							_this.get_node(event.target).get(0),
							_this,
						]);
						event.preventDefault();
						event.stopPropagation();
					...
					.live("contextmenu", function (event) {
						...
						return _this.callback("onrgtclk", [
							_this.get_node(event.target).get(0),
							_this,
							event,
						]);
					...
					.live("mousedown", function (event) {
						if (
							_this.settings.rules.drag_button == "left" &&
							event.which &&
							event.which != 1
						)
							return true;
						if (
							_this.settings.rules.drag_button == "right" &&
							event.which &&
							event.which != 3
						)
							return true;
						...
						if (
							...
							obj.children("a:eq(0)").hasClass("clicked")
						) {
							var counter = 0;
							for (var i in _this.selected_arr) {
								...
								if (
									...
									tree_component.drag_drop.origin_tree =
										_this;
									counter++;
								}
							}
							if (counter > 0) {
								...
								var tmp =
									tree_component.drag_drop.drag_node.clone();
								if (_this.settings.languages.length > 0)
									tmp.find("a")
										.not("." + _this.current_lang)
										.hide();
								tree_component.drag_drop.drag_help
									.children("ul:eq(0)")
									.append(tmp);
								tree_component.drag_drop.drag_help
									...
									.children("ul")
									.remove();
								tree_component.drag_drop.dragged =
									_this.container.find("li.dragged");
							}
						} else {
							if (_this.check("draggable", obj)) {
								tree_component.drag_drop.drag_node = obj;
			...
			off_height: function () {
				if (this.offset === false) {
					...
					var tmp = 0;
					tmp = parseInt(
						$.curCSS(this.container.get(0), "paddingTop", true),
						10,
					);
					if (tmp) this.offset.top += tmp;
					tmp = parseInt(
						$.curCSS(this.container.get(0), "borderTopWidth", true),
						10,
					);
					if (tmp) this.offset.top += tmp;
			...
			scroll_check: function (x, y) {
				...
				if (y - off.top < 20)
					cnt.scrollTop(
						Math.max(st - _this.settings.ui.scroll_spd, 0),
					);
				if (cnt.height() - (y - off.top) < h_cor)
					cnt.scrollTop(st + _this.settings.ui.scroll_spd);
				if (x - off.left < 20)
					cnt.scrollLeft(
						Math.max(sl - _this.settings.ui.scroll_spd, 0),
					);
			...
			scroll_into_view: function (obj) {
				...
				if (off_t + 5 < beg_t)
					this.container.scrollTop(
						this.container.scrollTop() - (beg_t - off_t + 5),
					);
				if (off_t + h_cor > end_t)
					this.container.scrollTop(
						this.container.scrollTop() + (off_t + h_cor - end_t),
					);
			},
			...
			check_move: function (nod, ref_node, how) {
				...
				var p =
					how != "inside"
						? this.parent(ref_node)
						: this.get_node(ref_node);
				nod = this.get_node(nod);
				if (p == false) return false;
				var r = {
					max_depth: this.settings.rules.use_max_depth
						? this.check("max_depth", p)
						: -1,
					max_children: this.settings.rules.use_max_children
						? this.check("max_children", p)
						: -1,
					valid_children: this.check("valid_children", p),
				};
				var nod_type =
					typeof nod == "string" ? nod : this.get_type(nod);
				if (
					typeof r.valid_children != "undefined" &&
					(r.valid_children == "none" ||
						(typeof r.valid_children == "object" &&
							$.inArray(
								nod_type,
								$.makeArray(r.valid_children),
							) == -1))
				)
					return false;
				if (this.settings.rules.use_max_children) {
					if (
						typeof r.max_children != "undefined" &&
						r.max_children != -1
					) {
						...
						if (
							...
							c_count =
								c_count - p.find("> ul > li.dragged").size();
						}
						if (
							r.max_children <
							p.find("> ul > li").size() + c_count
						)
							return false;
					}
				}
				...
				if (
					this.callback("check_move", [nod, ref_node, how, this]) ==
					false
				)
					return false;
				return true;
			...
			select_branch: function (obj, multiple) {
				...
				if (!_this.check("clickable", obj))
					return this.error("SELECT: NODE NOT SELECTABLE");
				if (
					_this.callback("beforechange", [obj.get(0), _this]) ===
					false
				)
					return this.error("SELECT: STOPPED BY USER");
				if (
					this.settings.rules.multiple != false &&
					multiple &&
			...
			open_branch: function (obj, disable_animation, callback) {
				...
				if (this.settings.data.async && obj.find("li").size() == 0) {
					if (
						this.callback("beforeopen", [obj.get(0), this]) ===
						false
					)
			...
			close_branch: function (obj, disable_animation) {
				...
				if (
					_this.callback("beforeclose", [obj.get(0), _this]) === false
				)
				...
				if (
					...
					if (
						this.settings.ui.selected_parent_close ==
							"select_parent" &&
						obj.children("a.clicked").size() == 0
					)
						this.select_branch(
			...
			set_lang: function (i) {
				if (
					!$.isArray(this.settings.languages) ||
					this.settings.languages.length == 0
				)
					return false;
				if (this.locked) return this.error("LOCKED");
				if (
					!$.inArray(i, this.settings.languages) &&
			...
			create: function (obj, ref_node, position) {
				...
				if (!obj.attributes[this.settings.rules.type_attr])
					obj.attributes[this.settings.rules.type_attr] =
						this.get_type(tmp) || "default";
					...
					for (var i = 0; i < this.settings.languages.length; i++) {
						if (!obj.data[this.settings.languages[i]])
							obj.data[this.settings.languages[i]] =
								(typeof this.settings.lang
									.new_node).toLowerCase() != "string" &&
								this.settings.lang.new_node[i]
									? this.settings.lang.new_node[i]
				...
				var r = {
					max_depth: this.settings.rules.use_max_depth
						? this.check("max_depth", root ? -1 : ref_node)
						: -1,
					max_children: this.settings.rules.use_max_children
						? this.check("max_children", root ? -1 : ref_node)
						: -1,
					valid_children: this.check(
						"valid_children",
						root ? -1 : ref_node,
					),
				};
				...
				if (
					typeof r.valid_children != "undefined" &&
					(r.valid_children == "none" ||
						($.isArray(r.valid_children) &&
							$.inArray(nod_type, r.valid_children) == -1))
				)
				...
				if (
					(typeof position).toLowerCase() == "undefined" ||
					position == "inside"
				)
					position =
				...
				if (
					ref_node.children("ul").size() == 0 ||
					(root == true &&
						ref_node.children("ul").children("li").size() == 0)
				) {
					if (!root)
						var a = this.moved(
							$li,
							ref_node.children("a:eq(0)"),
							"inside",
							true,
						);
					else
			...
			remove: function (obj) {
				...
				if (
					obj &&
					(!this.selected ||
						this.get_node(obj).get(0) != this.selected.get(0))
				) {
					obj = this.get_node(obj);
					if (obj.size()) {
			...
			prev: function (obj, strict) {
				...
				if (obj.prev("li").size()) {
					...
					return obj.parents("li:eq(0)").size()
						? obj.parents("li:eq(0)")
						: false;
			},
			parent: function (obj) {
			...
			callback: function (cb, args) {
				...
				for (var i in this.settings.plugins) {
					...
					if (typeof r !== "undefined" && r !== null) {
						if (cb == "ondata" || cb == "onparse") args[0] = r;
						else return r;
					}
				}
			...
			moved: function (what, where, how, is_new, is_copy, rb) {
				...
				if (!rb) {
					...
					if (!is_new) {
						var tmp =
							what.size() > 1
								? what.eq(0).parents(".tree:eq(0)")
								: what.parents(".tree:eq(0)");
						...
						if (tmp.settings.languages.length) {
							...
							if (this.settings.languages.length == 0)
								...
								for (var i in this.settings.languages) {
									if (
										!this.settings.languages.hasOwnProperty(
											i,
										)
									)
										continue;
									for (var j in tmp.settings.languages) {
										if (
											!tmp.settings.languages.hasOwnProperty(
												j,
											)
										)
											continue;
				...
				if (is_copy)
					...
					this.callback("oncreate", [
						this.get_node(what).get(0),
						$where.is("ul") ? -1 : this.get_node(where).get(0),
						how,
						this,
						rb,
					]);
			...
			paste: function (obj, position) {
				...
				if (
					...
					obj.parents()
						...
						.each(function () {
							if (
								tree_component.cut_copy.cut_nodes.index(this) !=
								-1
							) {
								ok = false;
								return false;
	...
	tree_component.mouseup = function (event) {
		...
		if (tmp.foreign === false && tmp.drag_node && tmp.drag_node.size()) {
			...
			if (tmp.move_type) {
				var tree1 =
					tree_component.inst[
						tmp.ref_node.parents(".tree:eq(0)").attr("id")
					];
				if (tree1)
					tree1.moved(
						tmp.dragged,
						...
						tmp.move_type,
						false,
						tmp.origin_tree.settings.rules.drag_copy == "on" ||
							(tmp.origin_tree.settings.rules.drag_copy ==
								"ctrl" &&
								event.ctrlKey),
					);
			}
		...
		if (tmp.foreign !== false) {
			...
			if (tmp.move_type) {
				...
				if (tree1)
					tree1.callback("ondrop", [
						tmp.f_data,
						tree1.get_node(tmp.ref_node).get(0),
						tmp.move_type,
						tree1,
					]);
			}
	...
	tree_component.mousemove = function (event) {
		...
		if (tmp.isdown) {
			if (
				!tmp.moving &&
				Math.abs(tmp.init_x - event.pageX) < 5 &&
			...
			if (event.target.tagName == "A" || event.target.tagName == "INS") {
				...
				var goTo = { x: et_off.left - 1, y: event.pageY - et_off.top };
				var arr = [];
				...
				var nn =
					tmp.foreign == false
						? tmp.origin_tree.container.find("li.dragged")
						: tmp.f_type;
				...
				if (ok) {
					switch (mov) {
						case "before":
							goTo.y = et_off.top - 2;
							tree_component.drag_drop.marker.attr(
								"class",
								"marker",
							);
							break;
					...
					tree_component.drag_drop.marker
						.css({ left: goTo.x, top: goTo.y })
						.show();
				}
			}
			if (
				...
				tree_component.drag_drop.marker
					.css({ left: et_off.left + 10, top: et_off.top + 15 })
					.show();
			} else if (
				(event.target.tagName != "A" &&
					event.target.tagName != "INS") ||
	...
	tree_component.cut_copy = { copy_nodes: false, cut_nodes: false };
	...
	tree_component.add_css = function (rule_name) {
		...
		tree_component.css.insertRule
			? tree_component.css.insertRule(rule_name + " { }", 0)
			: tree_component.css.addRule(rule_name, null, 0);
	...
	$.extend($.tree.datastores, {
		html: function () {
			return {
				get: function (obj, tree, opts) {
					return obj && $(obj).size()
						? $("<div>").append(tree.get_node(obj).clone()).html()
						: tree.container.children("ul:eq(0)").html();
				},
		...
		json: function () {
			return {
					...
					if (tree.settings.languages.length) {
						for (var i in tree.settings.languages) {
							...
							if (
								...
								json.data[tree.settings.languages[i]].title =
									tree.get_text(
										obj,
										tree.settings.languages[i],
									);
								if (
									a.children("ins").get(0).style.className
										.length
								) {
									json.data[tree.settings.languages[i]].icon =
										a
											.children("ins")
											.get(0).style.className;
								}
								if (
									a.children("ins").get(0).style
								...
								if (opts.inner_attrib.length) {
									...
									for (var j in opts.inner_attrib) {
										if (
											!opts.inner_attrib.hasOwnProperty(j)
										)
											continue;
										...
										if (
											...
											json.data[
												tree.settings.languages[i]
											].attributes[opts.inner_attrib[j]] =
												val;
										delete val;
									}
								}
						...
						if (opts.inner_attrib.length) {
							...
							for (var j in opts.inner_attrib) {
								...
								if (
									...
									json.data.attributes[opts.inner_attrib[j]] =
										val;
								delete val;
							}
						}
				...
				parse: function (data, tree, opts, callback) {
					if (
						Object.prototype.toString.apply(data) ===
						"[object Array]"
					) {
						var str = "";
					...
					if (tree.settings.languages.length) {
						for (
							var i = 0;
							i < tree.settings.languages.length;
							i++
						) {
							var attr = {};
							attr["href"] = "";
							...
							if (
								...
								(typeof data.data[tree.settings.languages[i]]
						...
						attr["class"] = "";
						if (
							(typeof data.data.attributes).toLowerCase() !=
							"undefined"
						) {
						...
						if (data.data.icon) {
							str +=
								"<ins " +
								(data.data.icon.indexOf("/") == -1
									? " class='" + data.data.icon + "' "
									: " style='background-image:url(\"" +
										data.data.icon +
										"\");' ") +
								"> </ins>";
						...
						str +=
							((typeof data.data.title).toLowerCase() !=
							"undefined"
								? data.data.title
								: data.data) + "</a>";
					}
