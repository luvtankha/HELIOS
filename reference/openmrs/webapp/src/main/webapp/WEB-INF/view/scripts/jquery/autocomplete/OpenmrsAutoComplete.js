...
function CreateCallback(options) {
	 ...
	 */
	this.personCallback = function () {
		var thisObject = this;
		return function (q, response) {
			if (jQuery.trim(q).length == 0) return response(false);

			thisObject.searchCounter += 1;
			DWRPersonService.findPeopleByRoles(
				q,
				false,
				options.roles,
				thisObject.makeRows(
					q,
					response,
					thisObject.searchCounter,
					thisObject.displayPerson,
				),
			);
		};
	};
	 ...
	 */
	this.patientCallback = function () {
		var thisObject = this;
		return function (q, response) {
			if (jQuery.trim(q).length == 0) return response(false);

			thisObject.searchCounter += 1;
			DWRPatientService.findPatients(
				q,
				false,
				thisObject.makeRows(
					q,
	...
	this.userCallback = function () {
		...
		return function (q, response) {
			...
			var rolesArray = [];
			if (options.roles.length > 0) rolesArray = options.roles.split(",");

			thisObject.searchCounter += 1;
			DWRUserService.findUsers(
				q,
				rolesArray,
				false,
				thisObject.makeRows(
	...
	this.conceptCallback = function () {
		...
		return function (q, response) {
			...
			thisObject.searchCounter += 1;
			DWRConceptService.findConcepts(
				q,
				false,
				includeClasses,
				excludeClasses,
	 ...
	 * none (yet)
	 */
	this.drugCallback = function () {
		var thisObject = this;
		return function (q, response) {
			if (jQuery.trim(q).length == 0) return response(false);

			thisObject.searchCounter += 1;
			DWRConceptService.findDrugs(
				q,
				false,
				thisObject.makeRows(
					q,
	 ...
	 */
	this.conceptAnswersCallback = function () {
		var thisObject = this;
		return function (q, response) {
	 ...
	 * showAnswersFor: a concept id. if non-null the search space is restricted to the answers to the given concept id
	 */
	this.encounterCallback = function () {
		var thisObject = this;
		return function (q, response) {
			if (jQuery.trim(q).length == 0) return response(false);
			...
			// do NOT return false if no text given, instead should return all answers
			thisObject.searchCounter += 1;
			DWREncounterService.findBatchOfEncountersByPatient(
				q,
				options.patientId,
				false,
				null,
	 ...
	 */
	this.conceptReferenceTermCallback = function (sourceElement) {
		var thisObject = this;
		return function (q, response) {
			if (jQuery.trim(q).length == 0) return response(false);
			var sourceId = null;
			if (sourceElement) sourceId = sourceElement.value;

			thisObject.searchCounter += 1;
			DWRConceptService.findBatchOfConceptReferenceTerms(
				q,
				sourceId,
				null,
				maxresults,
				...
				thisObject.makeRows(
					...
					response,
					thisObject.searchCounter,
					thisObject.displayConceptReferenceTerm,
				),
			);
		};
	};
	 ...
	 */
	this.providerCallback = function () {
		var thisObject = this;
		return function (q, response) {
			if (jQuery.trim(q).length == 0) return response(false);

			thisObject.searchCounter += 1;
			DWRProviderService.findProvider(
				q,
				false,
				0,
				50,
	 ...
	 */
	this.makeRows = function (q, response, searchId, displayFunction) {
		var thisObject = this;
	...
	this.displayPerson = function (origQuery) {
	 ...
	 */
	this.displayProvider = function (origQuery) {
		return function (provider) {
			...
			if (provider.identifier) textShown += provider.identifier + " ";
	...
	this.displayNamedObject = function (origQuery) {
	...
	this.displayDrugObject = function (origQuery) {
		return function (item) {
			// dwr sometimes puts strings into the results, just display those
			if (typeof item == "string") return { label: item, value: "" };
	...
	this.displayEncounter = function (origQuery) {
		return function (enc) {
			// dwr sometimes puts strings into the results, just display those
			if (typeof enc == "string") return { label: enc, value: "" };
	...
	this.displayConceptReferenceTerm = function (origQuery) {
		return function (item) {
			// dwr sometimes puts strings into the results, just display those
			if (typeof item == "string") return { label: item, value: "" };
			...
			var textShown =
				" " +
				item.code +
				(item.name != null && $j.trim(item.name) != ""
					? " - " + item.name
					: "") +
				" [" +
				item.conceptSourceName +
				"]";
