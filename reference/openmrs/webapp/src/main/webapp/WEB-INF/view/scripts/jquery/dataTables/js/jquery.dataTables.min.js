...
(function ($) {
	...
	_oExt.oJUIClasses = {
		...
		sPageButtonStaticDisabled:
			"fg-button ui-state-default ui-state-disabled",
	...
	$.fn.dataTable = function (oInit) {
		...
		function _fnAddColumn(oSettings, oOptions, nTh) {
			...
			if (
				...
				if (
					typeof oSettings.aoPreSearchCols[iLength].bEscapeRegex ==
					"undefined"
				) {
			...
			for (var i = 0; i < aData.length; i++) {
				...
				if (
					...
					sThisType = _fnDetectType(
						oSettings.aoData[iThisIndex]._aData[i],
					);
		...
		function _fnGatherData(oSettings) {
			...
			if (oSettings.sAjaxSource === null) {
				nTrs =
					oSettings.nTable.getElementsByTagName("tbody")[0]
						.childNodes;
		...
		function _fnDrawHead(oSettings) {
			...
			if (nTfoot.length !== 0) {
				...
				for (i = 0, iLen = nTfs.length; i < iLen; i++) {
					...
					if (!oSettings.aoColumns[i].bVisible) {
						nTfs[i - iCorrector].parentNode.removeChild(
							nTfs[i - iCorrector],
						);
						iCorrector++;
					}
			...
			if (typeof oSettings._bInitComplete == "undefined") {
				...
				if (
					...
					oSettings.nTable.style.width =
						oSettings.nTable.offsetWidth + "px";
				}
			}
		}
		...
		function _fnSort(oSettings, bApplyClasses) {
			...
			if (
				...
				if (!window.runtime) {
					var fnLocalSorting;
					var sDynamicSort =
						"fnLocalSorting = function(a,b){var iTest;";
					...
					iDataSort =
						oSettings.aoColumns[aaSort[aaSort.length - 1][0]]
							.iDataSort;
					iDataType = oSettings.aoColumns[iDataSort].sType;
					sDynamicSort +=
						"iTest = oSort['" +
					...
					for (i = 0; i < iColumns; i++) {
						if (nTds[i].className.indexOf(sClass + "1") != -1) {
							...
							if (nTds[i].className.indexOf(sClass + "2") != -1) {
								...
								if (
									nTds[i].className.indexOf(sClass + "3") !=
									-1
								) {
									for (
										j = 0, jLen = nTds.length / iColumns;
				...
				if (sAction == "previous") {
					...
					if (sAction == "next") {
						...
						if (sAction == "last") {
							...
							alert(
								"DataTables warning: unknown paging action: " +
									sAction,
							);
						}
					}
		...
		function _fnCalculateColumnWidths(oSettings) {
			...
			if (
				...
				for (i = 0; i < iColums; i++) {
					if (oSettings.aoColumns[i].bVisible) {
						sCalcHead +=
							"<th>" + oSettings.aoColumns[i].sTitle + "</th>";
						if (oSettings.aoColumns[i].sWidth !== null) {
							var sWidth = "";
							if (oSettings.aoColumns[i].sWidth !== null) {
								sWidth =
			...
			for (iRow = 0, iRows = nTrs.length; iRow < iRows; iRow++) {
				...
				for (
					...
					if (oSettings.aoColumns[iColumn].bVisible) {
						...
						anReturn.push(
							oSettings.aoData[iRow]._anHidden[iColumn],
						);
						iCorrector++;
					}
				}
		...
		function _fnSaveState(oSettings) {
			...
			sValue +=
				...
				'",';
			sValue +=
				'"sFilterEsc": ' + oSettings.oPreviousSearch.bEscapeRegex + ",";
			sValue += '"aaSorting": [ ';
