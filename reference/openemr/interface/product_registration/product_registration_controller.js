...
function ProductRegistrationController() {
  ...
  const _registrationCreatedHandler = function (data) {
    ...
    if (
      data &&
      typeof data === "object" &&
      Object.prototype.hasOwnProperty.call(data, "telemetry_enabled") &&
  ...
  const _formSubmissionHandler = function () {
    ...
    if (email > "" && email.indexOf("@") < 0) {
      $(".product-registration-modal .message").text(
        registrationTranslations.pleaseProvideValidEmail,
      );
    ...
    let allowTelemetry = $(".product-registration-modal #allowTelemetry").is(
      ":checked",
    )
      ? 1
      : 0;
    ...
    _productRegistrationService.submitRegistration(
      formData,
      function (err, data) {
        if (err) {
  ...
  const _displayFormView = function () {
    ...
    $(".product-registration-modal .modal-header").text(
      registrationTranslations.title,
    );
