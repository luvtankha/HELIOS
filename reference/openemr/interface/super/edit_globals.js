...
(function (window, oeUI) {
  ...
  function Widget(widgetContainer) {
    ...
    _widget.updateSortOrderInput = function () {
      let container = _widget.widgetContainer.querySelector(
        ".gbl-field-multi-sorted-list-container",
      );

      if (!container) {
        console.error(
          "Failed to find DOM Node with class .gbl-field-multi-sorted-list-container for widget " +
            WIDGET_NAME,
        );
        return;
      }
      ...
      if (options && options.length) {
        options.forEach(function (item) {
          inputText.push(item.dataset.optionId);
        });
      }
      let sortOrderInput = _widget.widgetContainer.querySelector(
        ".gbl-field-multi-sorted-list-value",
      );
      if (!sortOrderInput) {
        console.error(
          "Failed to find DOM Node with class .gbl-field-multi-sorted-list-value for widget " +
            WIDGET_NAME,
        );
        return;
      }
    ...
    _widget.addSelectedListOption = function (event) {
      ...
      if (!container) {
        console.error(
          "Failed to find container with class .gbl-field-multi-sorted-list-container in widget " +
            WIDGET_NAME,
        );
        return;
      }

      let itemNode = container.querySelector(
        '[data-option-id="' + target.value + '"]',
      );
      if (!itemNode) {
        let template = _widget.widgetContainer.querySelector(
          ".gbl-field-multi-sorted-list-item-template",
        );
        if (!template) {
          console.error(
            "Failed to find template with class .gbl-field-multi-sorted-list-item-template in widget " +
              WIDGET_NAME,
          );
          return;
        }
        ...
        if (!itemNode) {
          console.error(
            "Failed to find node from template with class .gbl-field-multi-sorted-list-item in widget " +
              WIDGET_NAME,
          );
          return;
        }
        ...
        if (target.options[target.selectedIndex] && textLabel) {
          ...
          console.error(
            "textLabel or selected item was missing in widget " + WIDGET_NAME,
          );
        }
      }
      ...
      let emptyList = container.parentNode.querySelector(
        ".gbl-field-multi-sorted-list-empty",
      );
      if (!emptyList) {
        console.error(
          "Failed to find DOM Node with class .gbl-field-multi-sorted-list-empty in widget " +
            WIDGET_NAME,
        );
      } else {
    ...
    _widget.init = function () {
      let select = _widget.widgetContainer.querySelector(
        ".gbl-field-multi-sorted-list-picker",
      );
      if (!select) {
        console.error(
          "Failed to find select node in DOM to initialize " + WIDGET_NAME,
        );
        return;
      }
      ...
      let items = container.querySelectorAll(
        ".gbl-field-multi-sorted-list-item",
      );
    ...
    _widget.destroy = function () {
      if (widgetItems && widgetItems.length) {
        widgetItems.forEach(function (w) {
          w.destroy();
        });
        widgetItems = [];
      }
      if (_widget.picker) {
        _widget.picker.removeEventListener(
          "change",
          _widget.addSelectedListOption,
        );
        _widget.picker = null;
      }
    };
  ...
  function WidgetItem(node, actionCallback) {
    ...
    function moveWidgetUp() {
      ...
      if (_widgetItem.node.previousElementSibling) {
        ...
        _widgetItem.node.parentNode.insertBefore(
          _widgetItem.node,
          _widgetItem.node.previousElementSibling,
        );
      } else {
    ...
    function moveWidgetDown(event) {
      ...
      if (_widgetItem.node.nextElementSibling) {
        ...
        _widgetItem.node.parentNode.insertBefore(
          _widgetItem.node,
          _widgetItem.node.nextElementSibling.nextElementSibling,
        );
      } else {
        console.error("moveWidget");
      }
    ...
    _widgetItem.init = function () {
      ...
      if (btnRemove) {
        ...
        console.error(
          "Failed to find DOM Node with class .btn-cancel for widget " +
            WIDGET_NAME,
        );
      }
      ...
      if (btnMoveUp) {
        ...
        console.error(
          "Failed to find DOM Node with class .btn-m-uparrow for widget " +
            WIDGET_NAME,
        );
      }
  ...
  function init() {
    let widgetNodes = window.document.querySelectorAll(
      ".gbl-field-multi-sorted-list-widget",
    );
...
window.document.addEventListener(
  "DOMContentLoaded",
  window.oeUI.multiSortedListWidget.init,
);
...
(function (window, oeUI) {
  class AddressBookWidget {
    ...
    updateUserDetails(userId, label) {
      this.#api
        .fetchStandardApiJSON("user?id=" + encodeURIComponent(userId))
        .then((userData) => {
      ...
      containers.forEach((container) => {
        ...
        if (input && button && label) {
          let popupClickHandler = () => {
            let url =
              top.webroot_url +
              "/interface/usergroup/addrbook_list.php?popup=2";
