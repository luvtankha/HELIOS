...
function find_all() {
  ...
  match = keys.match(/[\^$.|?+()\\~`!@#%&+={}<>]{1,}/);
  if (match) {
    document.getElementById("alert_msg").textContent = xl(
...
function get_word_count(form_id, form_dir, keys, case_sensitive) {
  ...
  var tokens = trimmed.split(/[\s,]+/).filter(function (t) {
    return t.length > 0;
  });
...
function next_prev(action) {
  ...
  if (w_count < 1) {
    ...
    if (tot_res > 0) {
      document.getElementById("alert_msg").textContent =
        xl("Showing result") + " " + cur_res + " " + xl("of") + " " + tot_res;
    }
  }
}
