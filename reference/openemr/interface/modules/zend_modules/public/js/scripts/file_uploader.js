...
$(function () {
  ...
  $(".file-uploader-progress-expand").click(function () {
    ...
    $(this)
      .parents()
      .eq(1)
      .css("height", progress_div_height + "px");
...
function AjaxFileUploader(settings) {
  ...
  if (settings.uploader_type == "single") {
    $("body").prepend(
      "<div id='" +
        uplader_id +
        "_drop' class='disable-select file-uploader-drop-div' style='width:" +
        window_width +
        "px;height:" +
        window_height +
        "px;'></div>",
...
function handleFileUpload(files, obj, settings) {
  ...
  for (var i = 0; i < file_length; i++) {
    ...
    fd.append(
      "batch_upload",
      files.length > 1 && settings.batch_upload ? 1 : 0,
    );
  ...
  var jqXHR = $.ajax({
    ...
    success: function (data) {
      ...
      if (typeof window[success_function] === "function") {
        ...
        console.warn(
          "file_uploader: callback not found on window:",
          success_function,
        );
      }
    },
...
function createStatusbar(obj) {
  ...
  this.abort = $(
    "<div class='abort'>" + resultTranslated.msg + "</div>",
