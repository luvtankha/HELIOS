...
$(function () {
    ...
    drop: function () {
      ...
      if (arr_id[1] == 0) {
        ...
        if (dragged_from_denied) {
          ...
          new_opacity =
            target_opacity == "1" && target_visibility != "none"
              ? "1"
              : source_opacity;
              ...
              $("#li_" + selected_user)
                ...
                .each(function () {
                  if ($(this).css("display") == "list-item") {
                    id = $(this)
                      .attr("id")
                      .replace("user_group_denied_", "user_group_allowed_");
        ...
        if (
          allowed_array.indexOf("li_" + new_id) == -1 &&
          allowed_array.indexOf(parent_id) == -1
        )
          allowed_array.push("li_" + new_id);
        ...
        if (
          new_id.substr(-2) != "-0" &&
          (allowed_array.indexOf(parent_id) == -1 || !dragged_from_denied)
        ) {
          $(
            $(
              $("#li_" + new_id)
        ...
        if (
          li_cout == 0 &&
          $(
            "#" + parent_id.replace("li_", "").replace("allowed", "denied"),
          ).css("opacity") != "1"
        ) {
          $(
            $(
  ...
  $(".droppableDenied").droppable({
    ...
    drop: function () {
      ...
      if (arr_id[1] == 0) {
              ...
              $("#li_" + selected_user)
                ...
                .each(function () {
                  if ($(this).css("display") == "list-item") {
                    id = $(this)
                      .attr("id")
                      .replace("user_group_allowed_", "user_group_denied_");
        ...
        if (
          denied_array.indexOf("li_" + new_id) == -1 &&
          denied_array.indexOf(parent_id) == -1
        )
          denied_array.push("li_" + new_id);
        ...
        if (
          new_id.substr(-2) != "-0" &&
          (denied_array.indexOf(parent_id) == -1 || !dragged_from_allowed)
        ) {
          $(
            $(
              $("#li_" + new_id)
        ...
        if (
          li_cout == 0 &&
          $(
            "#" + parent_id.replace("li_", "").replace("denied", "allowed"),
          ).css("opacity") != "1"
        ) {
          $(
            $(
  ...
  $(".module_check").click(function () {
    ...
    else $(".group_" + clicked_arr[1]).removeAttr("checked");
    saveGroupAcl();
  });
  ...
  $.ajax({
    ...
    success: function (thedata) {
      ...
      for (var index in obj["group_allowed"]) {
        $("#li_user_group_allowed_" + obj["group_allowed"][index] + "-0").css(
          "display",
          "",
        );
        $("#li_user_group_allowed_" + obj["group_allowed"][index] + "-0")
          .find("li")
          .css("display", "");
        allowed_array.push(
          "li_user_group_allowed_" + obj["group_allowed"][index] + "-0",
      ...
      for (var index in obj["group_denied"]) {
        $("#li_user_group_denied_" + obj["group_denied"][index] + "-0").css(
          "display",
          "",
        );
        $("#li_user_group_denied_" + obj["group_denied"][index] + "-0")
          .find("li")
          .css("display", "");
        denied_array.push(
          "li_user_group_denied_" + obj["group_denied"][index] + "-0",
      ...
      for (var index in obj["user_allowed"]) {
        if (
          $("#li_user_group_allowed_" + index + "-0").css("display") == "none"
        ) {
          $("#li_user_group_allowed_" + index + "-0").css("display", "");
          $("#li_user_group_allowed_" + index + "-0")
            .find("div:eq(1)")
            .css("opacity", "0.5");
        }
        for (var k in obj["user_allowed"][index]) {
          $(
            "#li_user_group_allowed_" +
              index +
              "-" +
              obj["user_allowed"][index][k],
          ).css("display", "");
          $(
            "#li_user_group_denied_" +
              index +
              "-" +
              obj["user_allowed"][index][k],
          ).css("display", "none");
      ...
      for (var index in obj["user_denied"]) {
        if (
          $("#li_user_group_denied_" + index + "-0").css("display") == "none"
        ) {
          $("#li_user_group_denied_" + index + "-0")
            .find("div:eq(1)")
            .css("opacity", "0.5");
