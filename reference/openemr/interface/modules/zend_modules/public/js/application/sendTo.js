  ...
  $.ajax({
    ...
    success: function (json) {
      ...
      for (form_id in components) {
        check_count++;
        checkBox +=
          "<div class='option_wrap'><input checked name='componentcheckbox[]' class='check_component' type='checkbox' value='" +
          form_id +
          "' ref='" +
          components[form_id] +
          "' >&nbsp;&nbsp;" +
...
function sendToEmrDirect() {
  ...
  var combination = "";
  var components = "";
  var downloadformat_type =
    $('input[name="downloadformat_type"]:checked').val() || "";
...
function sendToDownloadAll() {
  ...
  if (
    count == 0 &&
    $('input:radio[name="downloadformat"]:checked').val() != "qrda3" &&
    $('input:radio[name="downloadformat"]:checked').val() !=
      "qrda3_consolidated"
  ) {
    $(".ap-st-st-12").fadeOut();
    $(".activity_indicator").css({ display: "none" });
    ...
    const download_format = $(
      'input:radio[name="downloadformat"]:checked',
    ).val();
    const baseUrl =
      WEB_ROOT +
      "/interface/modules/zend_modules/public/encountermanager/index?";
...
function send() {
  ...
  if (send_to == "printer" || send_to == "fax") {
    ...
    if (send_to == "printer") {
      ...
      window.open(
        WEB_ROOT +
          "/interface/patient_file/encounter/print_report.php?" +
          params,
        "Print",
