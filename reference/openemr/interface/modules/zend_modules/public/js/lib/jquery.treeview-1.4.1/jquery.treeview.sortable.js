  ...
  $.widget(
    ...
    $.extend($.ui.mouse, {
      ...
      serialize: function (o) {
        var items = $(this.options.items, this.element).not(
          ".ui-sortableTree-helper",
        ); //Only the items of the sortable itself
        var str = [];
        o = o || {};

        items.each(function () {
          var res = ($(this).attr(o.attribute || "id") || "").match(
            o.expression || /(.+)[-=_](.+)/,
          );
          if (res)
      ...
      intersectsWithEdge: function (item) {
        var y1 = this.position.absolute.top - 10,
          y2 = y1 + 10;
        var t = item.top,
          b = t + item.height;
        ...
        if (
          !this.intersectsWith(
            item.item.parents(".ui-sortableTree").data("sortableTree")
              .containerCache,
          )
        )
          return false;
            ...
            if (!this.containers[i].containerCache.over) {
              if (this.currentContainer != this.containers[i]) {
                ...
                for (var j = this.items.length - 1; j >= 0; j--) {
                  if (
                    !this.containers[i].element[0].contains(
                      this.items[j].item[0],
                    )
                  )
                    continue;
      ...
      mouseStop: function (e) {
        if (this.newPositionAt)
          this.options.sortIndication.remove.call(
            this.currentItem,
            this.newPositionAt,
        ...
        if (!dropped && this.newPositionAt)
          this.newPositionAt[this.direction == "down" ? "before" : "after"](
            this.currentItem,
          ); //Append to element to its new position
        ...
        if (!this.element[0].contains(this.currentItem[0])) {
      ...
      rearrange: function (e, i, a) {
        if (i) {
          ...
          this.options.sortIndication[this.direction].call(
            this.currentItem,
            this.newPositionAt,
          );
        } else {
