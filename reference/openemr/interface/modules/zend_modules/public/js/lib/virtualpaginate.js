...
document.write(
  '<style type="text/css">' + //write out CSS for class ".hidepeice" that hides pieces of contents within pages
...
function virtualpaginate(config) {
  ...
  var elementType =
    typeof config.piececontainer == "undefined" ? "div" : config.piececontainer; //The type of element used to divide up content into pieces. Defaults to "div"
  this.pieces = virtualpaginate.collectElementbyClass(
    config.piececlass,
    elementType,
  ); //get total number of divs matching class name
  //Set this.chunksize: 1 if "chunksize" param is undefined, "chunksize" if it's less than total pieces available, or simply total pieces avail (show all)
  this.chunksize =
    typeof config.pieces_per_page == "undefined"
      ? 1
      : config.pieces_per_page > 0 &&
          config.pieces_per_page < this.pieces.length
        ? config.pieces_per_page
        : this.pieces.length;
  ...
  this.currentpage =
    typeof urlselectedpage == "number"
      ? urlselectedpage
      : this.persist
        ? persistedpage
        : config.defaultpage;
  this.currentpage =
    this.currentpage < this.pagecount ? parseInt(this.currentpage) : 0; //ensure currentpage is within range of available pages
...
virtualpaginate.prototype.navigate = function (keyword) {
  if (
    (!this.wraparound && keyword == "previous" && this.currentpage == 0) ||
    (!this.wraparound &&
      keyword == "next" &&
  ...
  else if (keyword == "first") this.currentpage = 0;
  else if (keyword == "last")
    this.currentpage = this.pagecount - 1; //last page number
  else this.currentpage = parseInt(keyword);
  ...
  for (var p = 0; p < this.paginatediv.length; p++) {
    //loop through all pagination DIVs
  ...
  if (!this.wraparound) {
    for (var i = 0; i < this.prevlinks.length; i++)
      //add or remove "disable" class from prev links depending on current page number
      virtualpaginate.setcssclass(
        this.prevlinks[i],
        "disabled",
        this.currentpage == 0 ? "add" : "remove",
      );
    for (var i = 0; i < this.nextlinks.length; i++)
      //add or remove "disable" class from next links depending on current page number
      virtualpaginate.setcssclass(
        this.nextlinks[i],
        "disabled",
        this.currentpage == this.pagecount - 1 ? "add" : "remove",
      );
  }
};
...
virtualpaginate.collectElementbyClass = function (classname, element) {
  ...
  if (document.querySelectorAll) {
    ...
    for (var i = 0; i < alltags.length; i++) {
      if (virtualpaginate.setcssclass(alltags[i], classname, "check"))
        //if element carries class name in question
...
virtualpaginate.urlparamselect = function (vpclass) {
  var result = window.location.search.match(
    new RegExp(vpclass + "=(\\d+)", "i"),
  ); //check for "?piececlass=2" in URL
...
virtualpaginate.prototype.showpage = function (pagenumber) {
  ...
  for (var i = 0; i < totalitems; i++) {
    if (i >= showstartindex && i <= showendindex) {
      ...
      var a = this.pieces[i];
      $(a).fadeIn();
    } else this.pieces[i].style.display = "none";
  }
  if (this.persist) {
    //if persistence enabled
    virtualpaginate.setCookie("dd_" + this.piececlass, this.currentpage);
  }
  if (this.cpspan.length > 0) {
    //if <span class="paginateinfo> element is present, update it with the most current info (ie: Page 3/4)
    for (var p = 0; p < this.cpspan.length; p++)
      this.cpspan[p].innerHTML =
...
virtualpaginate.prototype.paginate_build = function (
  divid,
  divpos,
  optnavtext,
) {
  ...
  if (paginatediv.getElementsByTagName("select").length > 0)
    //if there's a select menu in div
    this.paginate_build_selectmenu(
      paginatediv.getElementsByTagName("select")[0],
      divpos,
      optnavtext,
    );
  if (paginatediv.getElementsByTagName("a").length > 0)
    //if there are links defined in div
...
virtualpaginate.prototype.paginate_output_flatview = function (
  flatviewcontainer,
  divpos,
  anchortext,
) {
  ...
  for (var i = 0; i < this.pagecount; i++) {
    if (typeof anchortext[i] != "undefined")
      //if custom anchor text for this link exists
      flatviewhtml +=
        '<a href="#flatview" rel="' + i + '">' + anchortext[i] + "</a> "; //build pagination link using custom anchor text
    else
      flatviewhtml +=
        '<a href="#flatview" rel="' + i + '">' + (i + 1) + "</a> "; //build  pagination link using auto incremented sequential number instead
  }
  flatviewcontainer.innerHTML = flatviewhtml;
  this.paginate_build_flatview(flatviewcontainer, divpos, anchortext);
};

virtualpaginate.prototype.paginate_build_flatview = function (
  flatviewcontainer,
  divpos,
  anchortext,
) {
...
virtualpaginate.prototype.paginate_build_selectmenu = function (
  paginatedropdown,
  divpos,
  anchortext,
) {
  ...
  for (var i = 0; i < this.pagecount; i++) {
    if (typeof anchortext[i] != "undefined")
      //if custom anchor text for this link exists, use anchor text as each OPTION's text
      paginatedropdown.options[i] = new Option(anchortext[i], i);
    else
      //else, use auto incremented, sequential numbers
      paginatedropdown.options[i] = new Option(
        "Page " + (i + 1) + " of " + this.pagecount,
        i,
      );
  }
...
virtualpaginate.prototype.paginate_build_regularlinks = function (
  paginatelinks,
) {
  var instanceOfBox = this;
  for (var i = 0; i < paginatelinks.length; i++) {
    var currentpagerel = paginatelinks[i].getAttribute("rel");
    if (/^(previous)|(next)|(first)|(last)$/.test(currentpagerel)) {
      //screen for these "rel" values
    ...
    if (
      currentpagerel == "previous" ||
      paginatelinks[i].href.indexOf("previous") != -1
    ) {
      //check if this is a "previous" link
      if (!this.wraparound && this.currentpage == 0)
        //if current page is first page, disable "prev" link
        virtualpaginate.setcssclass(paginatelinks[i], "disabled", "add");
      this.prevlinks.push(paginatelinks[i]);
    } else if (
      currentpagerel == "next" ||
      paginatelinks[i].href.indexOf("next") != -1
    ) {
      //check if this is a "next" link
      if (!this.wraparound && this.currentpage == this.pagecount - 1)
        //if current page is last page, disable "next" link
        virtualpaginate.setcssclass(paginatelinks[i], "disabled", "add");
