...
function configure(id, imgpath) {
  if ($("#ConfigRow_" + id).css("display") != "none") {
    ...
    $.post("./Installer/configure", { mod_id: id }, function (data) {
      $(".config").hide();
      $("#ConfigRow_" + id).hide();
      $("#ConfigRow_" + id)
        .html(
          '<td colspan="10" style="background-color: var(--light);">' +
            data +
            "</td>",
        )
        .fadeIn();
...
function SaveMe(frmId, mod_id) {
  ...
  if (SelAccIndTab) var Acctitle = SelAccIndTab.panel("options").title;

  var SelTab = $("#tab" + mod_id).tabs("getSelected");
  if (SelTab) var Tabtitle = SelTab.panel("options").title;
      ...
      success: function (data) {
        $.each(data, function (jsonIndex, jsonValue) {
          if (jsonValue["return"] == 1) {
            $("#hook_response" + mod_id)
              .html(jsonValue["msg"])
              .fadeIn()
              .fadeOut(1000);
            $(function () {
              if (Tabtitle) $("#tab" + mod_id).tabs("select", Tabtitle);
            });
          }
        });
      },
