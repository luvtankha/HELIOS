document.getElementById("paynowbutton").onclick = function (e) {
  ...
  const createRainforestComponent = function (responseData) {
    ...
    component.addEventListener("approved", function (data) {
      ...
      alert(
        "Payment complete! It may take a few minutes to be reflected in the dashboard.",
      );
  ...
  const handleError = function () {
    alert(
      "An error occurred while preparing the payment. Your card has not been changed. Please try again.",
    );
    window.location.reload();
  };
