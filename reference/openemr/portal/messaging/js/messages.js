...
(function () {
  ...
  async function getAllMessages() {
    try {
      const data = await postForm(
        "handle_note.php",
        $.param({ task: "getall", csrf_token_form: state.csrf }),
      );
      if (data) {
  ...
  async function getDeletedMessages() {
    try {
      const data = await postForm(
        "handle_note.php",
        $.param({ task: "getdeleted", csrf_token_form: state.csrf }),
      );
      if (data) {
  ...
  async function getSentMessages() {
    try {
      const data = await postForm(
        "handle_note.php",
        $.param({ task: "getsent", csrf_token_form: state.csrf }),
      );
  ...
  async function deleteMessage(id) {
    try {
      await postForm(
        "handle_note.php",
        $.param({ task: "delete", noteid: id, csrf_token_form: state.csrf }),
      );
    } catch (e) {
      alert(e.message);
      throw e;
  ...
  function markReadById(id) {
    [state.items, state.inboxItems, state.sentItems, state.allItems].forEach(
      (list) => {
        if (!Array.isArray(list)) return;
        list.forEach((entry) => {
  ...
  function readAll() {
    ...
    const unread = state.items.filter(
      (item) => item && item.message_status === "New",
    );
    if (unread.length === 0) {
      return;
    }
  ...
  function renderNav() {
    document
      .querySelectorAll("#navInbox, #navSent, #navAll, #navTrash")
      .forEach((el) => {
    ...
    if (state.isSent)
      document.getElementById("navSent").classList.add("active");
  ...
  function renderMessageDetail() {
    ...
    chained.forEach((item) => {
      ...
      const showForward =
        isSelected && sel.sender_id !== state.cUserId && !state.isPortal;
    ...
    chainTbody
      .querySelectorAll(".btn-compose-reply, .btn-compose-fwd")
      .forEach((btn) => {
  ...
  function renderPaging() {
    ...
    const endNum =
      state.itemsPerPage * state.currentPage + currentPageItems.length;
    ...
    if (btnGroup) {
      btnGroup.style.display =
        state.items.length > state.itemsPerPage ? "" : "none";
    }
  }
  ...
  function setupComposeModal(e) {
    ...
    const relatedTarget =
      $(e.currentTarget).data("relatedTarget") || e.relatedTarget;
    ...
    const showRefer =
      (mode === "forward" || mode === "reply") &&
      state.selected &&
      state.selected.mail_chain;
    if (referMsgEl) {
      if (showRefer) {
    ...
    if (mode === "forward") {
      ...
      const referText = referMsgEl
        ? referMsgEl.textContent || referMsgEl.innerText || ""
        : "";
      const fmsg =
        "\n\n\n> " +
        state.xLate.fwd +
      ...
      $("#selForwardto").hide();
      $("#selSendto").show();
      $("#modalCompose .modal-header .modal-title").text(
        config.strings.composeReplyTitle,
      );
      ...
      $("#modalCompose .modal-header .modal-title").text(
        config.strings.composeNewTitle,
      );
      state.compose.task = "add";
  ...
  function submitForm(e) {
    ...
    if (task === "forward") {
      ...
      document.getElementById("hiddenSenderName").value =
        $fwd.text() || state.userproper;
      document.getElementById("hiddenRecipientId").value =
        state.compose.recipient_id || "";
      document.getElementById("hiddenRecipientName").value =
        state.compose.recipient_name || "";
      document.getElementById("hiddenPid").value = state.compose.pid || "";
      document.getElementById("hiddenNoteid").value =
        state.compose.noteid || "";
      ...
      document.getElementById("finputBody").value =
    ...
    document.getElementById("replyid").value =
      state.selected?.reply_mail_chain || "";
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    // Toolbar bindings
    document.getElementById("btnReadAll").addEventListener("click", (e) => {
      e.preventDefault();
      readAll();
    });
    document.getElementById("btnBatchDelete").addEventListener("click", (e) => {
      e.preventDefault();
      batchDelete();
    });
    ...
    if (state.isPortal) {
      document
        .querySelectorAll("[data-show-dashboard]")
