...
var page = {
    ...
    this.modelView.on("rendered", function () {
          ...
          if (page.portalpatient.get("pid")) {
            ...
            $.each(page.portalpatient.attributes, function (key, value) {
              if (value != page.patient.get(key)) {
                if (
                  ($("input[name=" + key + "]").attr("type") === "radio" ||
                    $("#" + key).is("select")) &&
                  value == ""
                ) {
                  value = "Unassigned";
                ...
                $("#" + key + "InputContainer span.help-inline").html(
                  '<a class="editval text-primary" onclick="page.toggleVal(this); return false;" data-tstate=new data-id="' +
                    _.escape(key) +
                    '">' +
                    _.escape(value) +
                    "</a>",
      ...
      $(function () {
        ...
        $("#dob").on("blur", function () {
          let bday = $(this).val() ?? "";
          let age = Math.round(
            Math.abs(new Date().getTime() - new Date(bday).getTime()),
          );
  ...
  replaceVal: function (el) {
    ...
    if (!document.getElementById(a)) {
      $(
        "input[name=" +
          a +
          '][value="' +
          _.escape(page.portalpatient.get(a)) +
          '"' +
          "]",
      )
        .prop("checked", true)
    ...
    $("#" + a + "InputContainer span.help-inline").html(
      '<a class="editval text-danger" style="font-size: 16px" onclick="page.revertVal(this); return false;" data-tstate=chart data-id="' +
        a +
        '">' +
        v +
        "</a>",
  ...
  revertVal: function (el) {
    ...
    $("#" + a + "InputContainer span.help-inline").html("");
    $("#" + a + "InputContainer span.help-inline").html(
      '<a class="editval text-primary" style="font-size: 16px" onclick="page.replaceVal(this); return false;" data-tstate="new" data-id="' +
        a +
        '">' +
        v +
        "</a>",
  ...
  updateModel: function (live = 0) {
    ...
    if (live !== 1) page.patient.urlRoot = "api/portalpatient";
    ...
    page.patient.save(
      ...
      },
      {
        ...
        success: function () {
          if (live !== 1) {
            setTimeout(
              "app.appendAlert('Patient was successfully " +
                (isNew ? "inserted" : "updated") +
                "','success',5000,'collectionAlert')",
              200,
            );
            setTimeout(
              "window.location.href ='" + webRoot + "/portal/home.php'",
              2500,
            );
          } else if (live === 1 && register !== "0") {
            // for testing
            //alert('Save Success');
          } else {
            window.dlgCloseAjax();
  ...
  deleteModel: function () {
    ...
    page.patient.destroy({
      ...
      success: function () {
        ...
        setTimeout(
          "app.appendAlert('The Patient record was deleted','success',5000,'collectionAlert')",
          500,
        );
        app.hideProgress("modelLoader");
