...
var page = {
  ...
  init: function () {
    ...
    if (!$.isReady && console) {
      console.warn(
        "page was initialized before dom is ready.  views may not render properly.",
      );
    }
    ...
    this.collectionView.on("rendered", function () {
      ...
      $("table.collection thead tr th")
        ...
        .on("click", function (e) {
          ...
          page.fetchParams.orderDesc =
            prop == page.fetchParams.orderBy && !page.fetchParams.orderDesc
              ? "1"
              : "";
    ...
    this.modelView.on("rendered", function () {
      ...
      $("#templatecontent").on(
        "focus",
        ".datetimepicker:not(.hasDatetimepicker)",
        function () {
      ...
      page.getDocument(
        page.onsiteDocument.get("docType"),
        cpid,
        page.onsiteDocument.get("filePath"),
      );
      if (page.isDashboard) {
        // review
      ...
      if (!isPortal) {
        ...
        isModule
          ? $(".dismissOnsiteDocumentButton").show()
          : $(".dismissOnsiteDocumentButton").hide();
        if (
          (isModule || page.isFrameForm || page.isDashboard) &&
          !page.isLocked &&
          page.currentName !== "Help"
        ) {
          $("#saveTemplate").show();
          $("#chartTemplate").show();
        } else {
        ...
        page.encounterFormName === "HIS" && !page.isLocked
          ? $("#chartHistory").show()
          : $("#chartHistory").hide();
          ...
          .on("click", function (e) {
            ...
            if (page.isFrameForm) {
              ...
              $(window).one("message onmessage", (e) => {
                ...
                let url = "";
                if (
                  page.encounterFormName.startsWith("LBF") ||
                  page.encounterFormName.startsWith("HIS")
                ) {
                  ...
                  if (!page.verifyValidEncounterForm(page.encounterFormName)) {
                    asyncAlertMsg(
                      "There is an issue loading form. Form does not exist.",
                    );
                    return false;
                  }
                  const params = new URLSearchParams({
                  ...
                  url =
                    webroot_url +
                    "/interface/forms/" +
                    encodeURIComponent(page.encounterFormName) +
                    "/patient_portal.php?" +
                    params;
                ...
                fetch(url)
                  ...
                  .then((content) => {
                    ...
                    content = templateContents.replace(
                      /(<iframe[^>]+>)/g,
                      content,
                    );
                    if (content) {
                ...
                fetch(url)
                  ...
                  .then((documentContents) => {
                    if (documentContents) {
                      ...
                      documentContents = templateContents.replace(
                        /(<iframe[^>]+>)/g,
                        documentContents,
                      );
          ...
          if (page.isFrameForm) {
            ...
            $(window).one("message onmessage", (e) => {
              ...
              if (page.onsiteDocument.get("denialReason") === "In Review") {
                ...
                pageAudit.onsitePortalActivity.set(
                  "pendingAction",
                  "patient submission",
                );
              }
          ...
          if (page.isFrameForm) {
            ...
            $(window).one("message onmessage", (e) => {
              ...
              if (autoRender + auditRender > 0) {
                ...
                location.assign(
                  webroot_url + "/portal/patient/onsitedocuments?" + params,
                );
              }
            });
            // post to submit and save content remote form.
      ...
      if (page.version === "Legacy" && isPortal && !page.isLocked) {
        alert(
          page.onsiteDocument.get("docType") +
            " is available for one last edit." +
            "\n" +
  ...
  getDocument: function (templateName, pid, template_id) {
    ...
    let currentNameStyled =
      page.currentName.substr(0, page.currentName.lastIndexOf(".")) ||
      page.currentName;
    currentNameStyled = currentNameStyled.replace(
      /[`~!@#$%^&*()_|+\-=?;:'",.<>\{\}\[\]\\\/]/gi,
      " ",
    );
    ...
    if (
      ...
      if (page.isFrameForm) {
        ...
        if ((m = regex.exec(templateContents)) !== null) {
          ...
          asyncAlertMsg(
            "There is an issue loading document. Missing Name Error.",
          );
          return false;
        }
        templateContents = templateContents.replace(
      ...
      $.ajax({
        ...
        success: function (templateHtml) {
          ...
          if (templateHtml.includes("Error") && autoRender + auditRender > 0) {
            ...
            asyncAlertMsg(
              xl("Onetime document is no longer available!") +
                "\n" +
                templateHtml,
              5000,
              "warning",
            ...
            asyncAlertMsg(
              xl("Sorry!") +
                ...
                templateHtml +
                "\n" +
                xl("Try to uncheck Activity table Show All."),
              5000,
              "danger",
            ).then((r) => {
              $("#Help").click();
              ...
              if (page.isFrameForm) {
                ...
                if (page.encounterFormName) {
                  ...
                  if (
                    ...
                    // first, ensure form name is valid
                    if (
                      !page.verifyValidEncounterForm(page.encounterFormName)
                    ) {
                      asyncAlertMsg(
                        "There is an issue loading form. Form does not exist.",
                      );
    ...
    if (page.currentName !== "Help") {
      $("#docPanelHeader").append(
        '<span class="bg-light text-dark px-1">' +
          jsText(currentNameStyled) +
          "</span>" +
          jsText(
            " " +
              page.version +
    ...
    if (page.onsiteDocument.id === null || page.onsiteDocument.id === "") {
      ...
      page.onsiteDocument.fetch({
        success: function () {
          if (
            page.isDashboard ||
            page.onsiteDocument.get("denialReason") === "Locked"
          ) {
  ...
  updateModel: function (reload = false, saveType = "") {
    ...
    let isLink = $("#patientSignature").attr("src")
      ? $("#patientSignature").attr("src").indexOf("signhere")
      : -1;
    let isWitnessLink = $("#witnessSignature").attr("src")
      ? $("#witnessSignature").attr("src").indexOf("signhere")
      : -1;
    if (isLink !== -1) {
    ...
    page.onsiteDocument.save(
      {
        pid: cpid,
        facility:
          page.formOrigin /* 0 portal, 1 dashboard, 2 patient documents */,
        ...
        createDate: page.onsiteDocument.get("createDate")
          ? page.onsiteDocument.get("createDate")
          : new Date(),
  ...
  deleteModel: function () {
    ...
    page.onsiteDocument.destroy({
      ...
      success: function () {
        ...
        pageAudit.onsitePortalActivity.set(
          "narrative",
          "Patient deleted un-charted template",
        );
        pageAudit.updateModel();
  ...
  initFileDrop: function (event) {
    return new Dropzone("#patientFileDrop", {
      ...
      clickable: true,
      acceptedFiles:
        "application/pdf,.png,.jpg,.jpeg,.doc,.docx,.xls,.xlsx,.csv,.tsv,.ppt,.pptx,.odt,.rtf",
