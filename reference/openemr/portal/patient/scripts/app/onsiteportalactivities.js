...
var pageAudit = {
  ...
  init: function () {
    ...
    if (!$.isReady && console)
      console.warn(
        "pageAudit was initialized before dom is ready.  views may not render properly.",
      );
  ...
  fetchOnsitePortalActivities: function (params, hideLoader) {
    ...
    if (pageAudit.fetchInProgress) {
      if (console)
        console.log(
          "pageAudit suppressing fetch because it is already in progress",
        );
    }
   ...
   */
  getAudit: function (m) {
    pageAudit.onsitePortalActivity = m
      ? m
      : new model.OnsitePortalActivityModel();

    pageAudit.modelView.model = pageAudit.onsitePortalActivity;

    if (
      pageAudit.onsitePortalActivity.id == null ||
      pageAudit.onsitePortalActivity.id === ""
    ) {
  ...
  updateModel: function () {
    ...
    pageAudit.onsitePortalActivity.save(
      ...
      },
      {
        ...
        success: function () {
          if (isNew) {
            pageAudit.onsitePortalActivities.add(
              pageAudit.onsitePortalActivity,
            );
          }
  ...
  deleteModel: function () {
    ...
    pageAudit.onsitePortalActivity.destroy({
      ...
      success: function () {
        ...
        setTimeout(
          "app.appendAlert('The OnsitePortalActivity record was deleted','alert-success',3000,'collectionAlert')",
          500,
        );
