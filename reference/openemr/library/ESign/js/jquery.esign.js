...
(function ($) {
  $.fn.esign = function (customSettings, customEvents) {
    ...
    var events = $.extend(
      {
        ...
        afterFormSuccess: function (response) {
          var logId =
            "esign-signature-log-" + response.formDir + "-" + response.formId;
          top.restoreSession();
          $.post(settings.logViewAction, response, function (html) {
            $("#" + logId).replaceWith(html);
          });
          var editButtonId =
            "form-edit-button-" + response.formDir + "-" + response.formId;
    ...
    $("body").append(
      "<div id='esign-mask-content' class='window rounded'></div>",
    );
    ...
    $(document).on(
      "click",
      "#esign-sign-button-" + settings.module,
      function (e) {
        e.preventDefault();
          ...
          function (response) {
            if (response.status != "success") {
              ...
              $("#esign-mask-content")
                .find("form")
                .append(
                  "<div id='esign-form-error-container' class='error'>" +
                    response.message +
                    "</div>",
                );
            } else {
    ...
    function initElement(element) {
      ...
      element.click(function (e) {
        ...
        //Set the popup window to center
        $("#esign-mask-content").css(
          "top",
          Math.min(y / 4 - $("#esign-mask-content").height() / 4, 100),
        );
        $("#esign-mask-content").css(
          "left",
          x / 2 - $("#esign-mask-content").width() / 2,
        );
