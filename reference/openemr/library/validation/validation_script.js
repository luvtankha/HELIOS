  ...
  function submitme(new_validate, e, form_id, constraints, g_date_format) {
    ...
    if (new_validate !== 1) {
      ...
      if (validate(f)) {
        ...
        //If there was an error prevent the form submit in order to display them
        e.preventDefault();
      }
    } else {
      //If the new validation library is used :
      ...
      for (var key in elements) {
        ...
        if (!$(element).is("select[multiple]")) {
          if (
            $(element).parent().prop("style") != undefined &&
            ($(element).parent().prop("style").visibility == "hidden" ||
      ...
      for (var key in elements) {
        ...
        if ($(element).is("select[multiple]")) {
          ...
          if (validate.isObject(constraints[new_key])) {
            ...
            if (
              constraints[new_key].presence &&
              (elements[key].length == 0 || elements[key][0] == null)
            ) {
      ...
      function appendError(input, id, message) {
        ...
        if ($("#error_" + id).length == 0) {
          ...
          if ($(input).next().length > 0) {
            $(input)
              .next()
              .after(
                "<span id='error_" + id + "' class='error-message' '></span>",
              );
          } else {
            $(input).after(
              "<span id='error_" + id + "' class='error-message'></span>",
        ...
        var error_msg =
          typeof constraints.custom_messages !== "undefined" &&
          constraints.custom_messages
            ? message
