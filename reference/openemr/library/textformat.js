...
function dateblur(e, defcc, withtime) {
  ...
  if (
    ...
    if (confirm("Date entry is incomplete! Try again?")) e.focus();
    else e.value = "";
    return;
  }
...
var twodigitccs =
  "/20/30/31/32/33/34/36/39/40/41/43/44/45/46/47/48/49/51/52/53/54/55/56/57/58/60/61/62/63/64/65/66/81/82/84/86/90/91/92/93/94/95/98/";
...
function maskblur(elem, mask) {
  ...
  if (i > 0 && v.length > 0 && v.length != i) {
    ...
    if (i > v.length) {
      ...
      if (confirm("Field entry is incomplete! Try again?")) elem.focus();
      else elem.value = "";
      return;
    }
