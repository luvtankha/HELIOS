    ...
    function includeScript(srcUrl, type) {
      return new Promise(function (resolve, reject) {
        if (type == "script") {
          ...
          console.log(
            "Needed to load:[" + srcUrl + "] For: [" + location + "]",
          );
        }
        if (type === "link") {
...
if (typeof alertMsg !== "function") {
  ...
  function alertMsg(
    ...
    $("body").prepend(
      `<div class="container text-center" id="alert_box" style="${jsAttr(style)}"></div>`,
    );
...
function dlgopen(url, winname, width, height, forceNewWindow, title, opts) {
  ...
  jQuery(function () {
    ...
    let bscss =
      top.webroot_url + "/public/assets/bootstrap/dist/css/bootstrap.min.css";
    let bscssRtl =
      top.webroot_url +
