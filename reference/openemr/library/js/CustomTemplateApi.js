...
function doTemplateEditor(_this, event, oContext = "") {
  ...
  let title =
    '<i class="fa fa-th"></i><h4 class="ml-2">' +
    xl("Text Templates") +
    "</h4>";
  ...
  let url =
    top.webroot_url + "/library/custom_template/custom_template.php?" + params;
  dlgopen(url, "", "modal-lg", 800, "", "", {
    buttons: [{ text: xl("Do Nothing"), close: true, style: "secondary" }],
    type: "iframe",
  });
  return false;
}
...
const bindTextArea = function () {
  const teventElement = document.querySelectorAll(
    "textarea, input[type='text']:not(.skip-template-editor)",
  );
