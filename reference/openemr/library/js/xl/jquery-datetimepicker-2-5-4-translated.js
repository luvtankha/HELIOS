...
function datetimepickerTranslated(selector, params) {
  ...
  let DateFormatRead =
    formatters.DateFormatRead ||
    function (mode = "legacy") {
      return "Y-m-d";
    };
  let TimeFormatRead =
    formatters.TimeFormatRead ||
    function (includeSeconds = true) {
      return "H:i:s";
    };
  ...
  if (params.timepicker) {
    if (params.showSeconds) {
      if (params.formatInput) {
        ...
        defaults.format =
          DateFormatRead("jquery-datetimepicker") + " " + time_format_with_secs;
      } else {
  ...
  if (languageDirection === "rtl") {
    defaults.onGenerate = function (current_time, $input) {
      ...
      if (position.left < datepickerPopupWidth) {
        ...
        $(".xdsoft_datetimepicker").offset({
          left: position.left - datepickerPopupWidth + $($input).innerWidth(),
        });
      }
    };
  }
