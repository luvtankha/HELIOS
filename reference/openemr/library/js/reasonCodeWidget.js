...
(function (window, oeUI) {
  ...
  function getNegationContainerFromWidget(widget) {
    ...
    if (!node) {
      console.error(
        "Failed to find reason widget container node with id " + containerId,
      );
    }
    return node;
  }
  ...
  function Widget(btnToggleNode) {
    ...
    this.destroy = function () {
      if (_this.reasonCodeInput) {
        _this.reasonCodeInput.removeEventListener(
          "click",
          _this.launchCodeSelector,
        );
      }
      if (_this.btnToggle) {
    ...
    function init() {
      if (_this.reasonCodeContainer) {
        _this.reasonCodeInput = _this.reasonCodeContainer.querySelector(
          ".code-selector-popup",
        );
        if (_this.reasonCodeInput) {
          _this.reasonCodeInput.addEventListener(
            "click",
            _this.launchCodeSelector,
          );
        } else {
          console.error(
            "Failed to find input node with selector .code-selector-popup",
          );
        }
        _this.reasonCodeText = _this.reasonCodeContainer.querySelector(
          ".code-selector-text-display",
        );
        _this.reasonCodeTextInput = _this.reasonCodeContainer.querySelector(
          ".code-selector-text",
        );
        _this.reasonStatusNode =
          _this.reasonCodeContainer.querySelector("select");
      } else {
  ...
  function destroyWidget(widget) {
    if (widget && widget.removeEventListener) {
      ...
      window.console.error(
        "Widget called to be destroyed but was not a valid widget",
        widget,
      );
    }
  }
