...
function checkLength(eleName, eleVal, len) {
  ...
  if (eleVal.length > len)
    m +=
      window.top.xl("Invalid length for") +
      " " +
      eleName.toUpperCase() +
      ". " +
      window.top.xl(
...
function checkSpecialCharacter(eleName, eleVal) {
  ...
  if (regE.test(eleVal) == true)
    m +=
      window.top.xl("Invalid character in") +
      " " +
      eleName.toUpperCase() +
      "\n";
  return m;
...
function checkPhone(eleName, eleVal) {
  var regE = /[^0-9']/;
  var m = "";
  eleVal_temp = eleVal.replace(/[^0-9]/gi, "");
  eleVal = eleVal.replace(/[-)(\s]/gi, "");
  eleName = eleName.replace("form_", "");
  eleName = eleName.replace("_", " ");
  if (regE.test(eleVal) == true)
    m +=
      window.top.xl("Invalid non-numeric character in") +
      " " +
      eleName.toUpperCase() +
      "\n";
    ...
    m +=
      eleName.toUpperCase() +
      " " +
      window.top.xl("should contain only 10 digits") +
      "\n";
  else if (eleVal_temp.length < 10)
    m +=
      eleName.toUpperCase() +
      " " +
      window.top.xl("should contain 10 digits") +
      "\n";
  return m;
}
