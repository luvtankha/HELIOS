 ...
 */
let { EditPolicyScreenController } = await import(
  "./EditPolicyScreenController.js?v=" + window.top.jsGlobals.assetVersion
);
let { NewPolicyScreenController } = await import(
  "./NewPolicyScreenController.js?v=" + window.top.jsGlobals.assetVersion
);
...
export class InsuranceEditMainController {
  ...
  constructor(csrfToken, apiURL, insuranceProviderList, types, puuid) {
    this.__insurancePolicyService = new InsurancePolicyService(
      csrfToken,
      apiURL,
  ...
  init() {
    this.__insurancePolicyService
      ...
      .catch((error) => {
        window.top.xl(
          "Failed to load insurance policies, contact a system administrator",
        );
        console.error(error);
      });
  }
  ...
  setupSaveDataAlert() {
    window.addEventListener("beforeunload", (event) => {
      let somethingChanged =
        ...
        this.__editPolicyScreen.hasDataToSave();
  ...
  setupEditScreen() {
    this.__editPolicyScreen = new EditPolicyScreenController(
      this.__csrfToken,
      this.__insurancePolicyService,
    );
    ...
    btnAddPolicy.addEventListener("click", () => {
      if (this.__editPolicyScreen.hasDataToSave()) {
        if (
          !confirm(
            window.top.xl(
              "Leaving this screen will discard your changes. Are you sure you want to leave?",
            ),
          )
        ) {
          return;
  ...
  setupNewPolicyScreen() {
    this.__newPolicyScreen = new NewPolicyScreenController(
      this.__insurancePolicyService,
    );
