...
export class InsurancePolicyModel {
  ...
  populate(pojo, markAsUnchanged = false) {
    if (typeof pojo === "object") {
      ...
      objectKeys.forEach((key) => {
        if (
          ["date", "date_end", "subscriber_DOB", "accept_assignment"].includes(
            key,
          )
        ) {
          return;
      ...
      this.setProperty(
        "date",
        this.#getValidFormattedDateFromServerDate(pojo.date),
      );
      this.setProperty(
        "date_end",
        this.#getValidFormattedDateFromServerDate(pojo.date_end),
      );
      this.setProperty(
        "subscriber_DOB",
        this.#getValidFormattedDateFromServerDate(pojo.subscriber_DOB),
      );
      if (markAsUnchanged) {
  ...
  toString() {
    ...
    if (this.date) {
      names.push(
        window.top.oeFormatters.I18NDateFormat(this.date, dateFormatSettingYMD),
      );
    } else {
