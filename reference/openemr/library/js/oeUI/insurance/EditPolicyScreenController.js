 ...
 */
let { InsurancePolicyService } = await import(
  "./InsurancePolicyService.js?v=" + window.top.jsGlobals.assetVersion
);
let { InsurancePolicyModel } = await import(
  "./InsurancePolicyModel.js?v=" + window.top.jsGlobals.assetVersion
);
let { ValidationFieldError, ValidationError } = await import(
  "../Error/ValidationError.js?v=" + window.top.jsGlobals.assetVersion
);
...
export class EditPolicyScreenController {
  ...
  constructor(csrfToken, insurancePolicyService) {
    ...
    this.__insuranceProviderList =
      insurancePolicyService.getInsuranceProvidersList();
  ...
  resetSaveData() {
    if (this.selectedInsurance) {
      let insurancesByType =
        this.__insurancesByType[this.__selectedInsuranceTypeTab];
      let resetInsurance = insurancesByType.find(
        (insurance) => insurance.id == this.selectedInsurance.id,
      );
      if (resetInsurance) {
  ...
  setupNewPolicyEdit(newPolicyData) {
    ...
    this.__insurancesByType =
      this.__insurancePolicyService.getInsurancesByType();
    this.setup();
  }
  ...
  #setupWindowEvents() {
    ...
    this.#boundSetupEvents.push({
      node: window,
      event: "focusin",
      callback: focusInHandler,
    });
      ...
      if (evt.data && typeof evt.data.action == "string") {
        if (evt.data.action == "insurance-patient-browser-selected") {
          ...
          if (!patientUuid || !insuranceUuid) {
            alert(
              window.top.xl("No patient was selected to copy values from."),
            );
            return;
          }
          ...
          let patientInsuranceObject = this.__insurancePolicyService
            .getPatientInsuranceData(patientUuid, insuranceUuid)
            .then((result) => {
            ...
            .catch((error) => {
              ...
              alert(
                window.top.xl(
                  "An error occurred while loading the insurance policy. Please try again or contact your system administrator.",
                ),
              );
            });
          this.render();
          ...
          if (!insuranceCompanyId || !insuranceCompanyName) {
            alert(
              window.top.xl(
                "An error occurred while loading the insurance company information.",
              ),
            );
            console.error(
              "Failed to find insurance company id or name in event data, this should not happen and is a bug. Event: ",
              evt,
            );
            return;
          }
          this.selectedInsurance.setProperty("provider", insuranceCompanyId);
          this.__insurancePolicyService.addInsuranceProviderToList(
            insuranceCompanyId,
            insuranceCompanyName,
          );
          this.__insuranceProviderList =
            this.__insurancePolicyService.getInsuranceProvidersList();
          this.render();
        }
      }
    };
  ...
  #updateSubscriberRelationshipWithPatientData() {
    ...
    this.__insurancePolicyService
      ...
      .catch((error) => {
        ...
        this.render();
        window.top.xl(
          "Could not retrieve patient data for subscriber relationship.",
        );
      });
  }
  #setupModelSyncBinding(selectedInsurance) {
    let type = selectedInsurance.type;
    let insuranceInfoContainer = document.getElementById(
      "insurance-info-type-" + type,
    );

    let keys = Object.keys(selectedInsurance);
    keys.forEach((key) => {
      let input = insuranceInfoContainer.querySelector(
        '[name="form_' + key + '"]',
      );
      if (input) {
    ...
    insuranceInfoContainer
      .querySelectorAll(".js-capitalize-me")
      .forEach((input) => {
    ...
    if (typeof window.policykeyup == "function") {
      ...
      insuranceInfoContainer
        .querySelectorAll(".js-policykeyup")
        .forEach((input) => {
    ...
    this.addCustomEventCleanupAction(
      { selector: "#" + insuranceInfoContainer.id + " .datepicker" },
      (evt) => {
    ...
    this.addCustomEventCleanupAction(
      { selector: "#" + insuranceInfoContainer.id + " .sel2" },
      (evt) => {
    ...
    insuranceInfoContainer
      .querySelector(".subscriber-relationship-open-finder")
      .addEventListener("click", (evt) => {
        ...
        window.open(
          dlgUrl,
          id,
          "toolbar=0,scrollbars=1,location=0,statusbar=0,menubar=0,resizable=1,width=400,height=450,left = 440,top = 362",
        );
      });

    insuranceInfoContainer
      .querySelector(".insurance-search-open-finder")
      .addEventListener("click", (evt) => {
        let input = evt.target;
        let url = new URL(input.href);
  ...
  #setupAddressValidation(selectedInsurance) {
    ...
    if (!insuranceInfoContainer) {
      throw new Error(
        "Failed to find insurance info container for type: " + type,
      );
    }
    ...
    if (template) {
      insuranceInfoContainer
        .querySelectorAll(".js-verify-address")
        .forEach((input) => {
  ...
  #verifyAddress(evt, address) {
    ...
    dlgopen(
      "../../practice/address_verify.php?" + params,
      "_blank",
      400,
      150,
      "",
  ...
  savePolicy() {
    ...
    this.render(); // re-render the screen to update the display
    this.__insurancePolicyService
      .saveInsurance(this.selectedInsurance)
      .then((result) => {
      ...
      .catch((error) => {
        if (error instanceof ValidationError) {
          ...
          console.error(error);
          alert(
            "An error occurred while saving the insurance policy.  Please try again or contact your system administrator.",
          );
        }
      })
      .finally(() => {
    ...
    if (navLinks) {
      navLinks.forEach((link) => {
        link.addEventListener("click", (evt) => {
          ...
          if (
            !this.__insurancesByType[this.__selectedInsuranceTypeTab].length
          ) {
            console.error(
              "Failed to find insurance for type: " +
                this.__selectedInsuranceTypeTab,
            );
          }
          this.#updateSelectedInsuranceObject(
            this.__insurancesByType[this.__selectedInsuranceTypeTab][0],
          );
          this.render();
        });
      });
  ...
  render() {
    ...
    let selectedInsuranceTypeAnchor = document.querySelector(
      '.nav-link-insurance-type[data-type="' + type + '"]',
    );
    let selectedInsuranceTypeTab = selectedInsuranceTypeAnchor.closest(
      ".nav-link-insurance-type-container",
    );
    ...
    document.querySelectorAll(".nav-link-insurance-type").forEach((l) => {
      l.closest(".nav-link-insurance-type-container").classList.remove(
        "current",
      );
    });
    ...
    let noPolicies = document.querySelector(
      ".insurance-type-no-policies-" + type,
    );
    let typeHasNoExistingPolicy =
      this.__insurancesByType[type].find(
        (insurance) => insurance.id != null,
      ) === undefined;
    let selectorRow = document.querySelector(
      ".insurance-type-selector-row-" + type,
    );
    ...
    if (select) {
      ...
      insurances.forEach((insurance) => {
        ...
        if (
          Object.prototype.hasOwnProperty.call(insurance, "end_date") &&
          insurance.end_date !== null
        ) {
          option.innerText +=
            " - " +
            window.top.xl("End Date") +
            ": " +
            window.top.oeFormatters.I18NDateFormat(
              insurance.end_date,
              displayFormatSettingYMD,
            );
        }
        select.appendChild(option);
      });
      if (this.selectedInsurance) {
        let selectedOptionInsuranceId =
          this.selectedInsurance.id === null ? "" : this.selectedInsurance.id;
        let option = select.querySelector(
          '[value="' + selectedOptionInsuranceId + '"]',
        );
        if (option) {
    ...
    if (this.selectedInsurance) {
      this.#populateInsuranceInformationForSelectedInsurance(
    ...
    let buttonTabContainer = document.querySelector(
      ".tabContainer .tab.current",
    );
    if (buttonTabContainer) {
      if (this.__isSaving) {
        buttonTabContainer
          .querySelector(".btn-saving")
          .classList.remove("d-none");
        buttonTabContainer
          .querySelector(".btn-save-policy")
      ...
      if (this.__policySaved) {
        buttonTabContainer
          .querySelector(".insurance-save-success")
          .classList.remove("d-none");
      } else {
        buttonTabContainer
          .querySelector(".insurance-save-success")
    ...
    if (this.__currentFocusedInputName) {
      ...
      let input = currentTab.querySelector(
        '[name="' + this.__currentFocusedInputName + '"]',
      );
      this.__currentFocusedInputName = null;
      if (input) {
        let tabbableInputs = currentTab.querySelectorAll(
          "a[href], button, input, select, textarea",
        );
        let currentInputIndex = Array.prototype.indexOf.call(
          tabbableInputs,
          input,
        );
  ...
  #renderValidationErrorMessages(selectedInsurance) {
    ...
    let markedFields = insuranceInfoContainer.querySelectorAll(
      ".invalid-input.error",
    );
    markedFields.forEach((field) => {
      ...
      if (errorNode && errorNode.classList.contains("sel2")) {
        ...
        errorNode = errorNode.nextElementSibling
          ? errorNode.nextElementSibling.querySelector(".select2-selection")
          : null;
      }
      if (errorNode) {
    ...
    if (!this.__validationErrors.length) {
      ...
      let templateNode = document.getElementById(
        "insurance-validation-error-template",
      );
      if (!templateNode) {
      ...
      let container =
        insuranceInfoContainer.querySelector(".validation-errors");
      if (!container) {
        throw new Error("Failed to find validation error container");
      }

      let detailsList = container.querySelector(
        ".insurance-validation-details-list",
      );
      if (!detailsList) {
      ...
      this.__validationErrors.forEach((error) => {
        ...
        for (let key in error.validationErrors) {
          ...
          if (!fieldContainer) {
            ...
            fieldContainer = container.querySelector(
              '[data-error-id="Unknown::UNKNOWN"]',
            );
            if (!fieldContainer) {
              throw new Error(
                "Failed to find validation container for field: " + key,
              );
            }
          }
          let clonedFieldContainer = fieldContainer.cloneNode(true);
          clonedFieldContainer.classList.remove("d-none");
          let validationFieldName = clonedFieldContainer.querySelector(
            ".validation-field-name",
          );
          if (!validationFieldName) {
            throw new Error(
              "Failed to find validation field name for validation field with key " +
                key,
            );
          }
          let validationFieldLabel = insuranceInfoContainer.querySelector(
          ...
          let fieldInput = insuranceInfoContainer.querySelector(
            '[name="form_' + field + '"]',
          );
    ...
    this.__types.forEach((type) => {
      ...
      if (select) {
        select.addEventListener("change", (evt) => {
          ...
          let updatedInsurance = this.__insurancesByType[type].find(
    ...
    if (selectedInsurance) {
      try {
        ...
        if (insuranceInfoContainer) {
          ...
          this.#populateInsuranceProviderListForNode(
            insuranceInfoContainer,
            this.__insuranceProviderList,
          );
          // the select2 setup needs to occur after the data elements have been populated.
          this.#populateInsuranceModelDataElements(
            insuranceInfoContainer,
            selectedInsurance,
          );
        ...
        alert(
          window.top.xl(
            "An error occurred while rendering the insurance policy. Please try again or contact your system administrator.",
          ),
        );
      }
    }
  ...
  #setupInsuranceSwapButtons(selectedInsurance) {
    ...
    let fieldDestination =
      insuranceInfoContainer.querySelector(".js-swap-insurance");
    if (!fieldDestination) {
      throw new Error("Failed to find swap insurance field for type: " + type);
    }
    let clone = document.importNode(template.content, true);
    fieldDestination.parentNode.insertBefore(
      clone,
      fieldDestination.nextSibling,
    );
    let swapButton = insuranceInfoContainer.querySelector(
      '.btn-insurance-swap[data-type="' + type + '"]',
    );
    if (!swapButton) {
    ...
    swapButton.addEventListener("click", (evt) => {
      ...
      this.__policySaved = false;
      this.__insurancePolicyService
        .swapInsurance(this.selectedInsurance, targetType)
        ...
        .catch((error) => {
          ...
          if (error instanceof ValidationError) {
            ...
            alert(
              window.top.xl(
                "An error occurred while swapping the insurance policy. Please try again or contact your system administrator.",
              ),
            );
          }
        });
  ...
  #isIgnoredModelProperty(property) {
    return (
      ["id", "insureruuid", "pid", "puuid", "type", "uuid"].indexOf(property) <
      0
    );
  }

  #populateInsuranceModelDataElements(
    insuranceInfoContainer,
    selectedInsurance,
  ) {
    let keys = Object.keys(selectedInsurance);
    keys.forEach((key) => {
      ...
      if (input.classList.contains("datepicker")) {
        ...
        if (option) {
          ...
          console.error(
            "Failed to find select option value for key: " +
              key +
              " value: " +
              value,
          );
      ...
      if (
        this.__updatingSubscriberSelfRelationship &&
        selectedInsurance.isPatientDataField(key)
      ) {
    ...
    let insuranceInfoType = insuranceInfoContainer.querySelector(
      ".insurance-info-type",
    );
