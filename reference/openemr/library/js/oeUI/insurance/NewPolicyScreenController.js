...
export class NewPolicyScreenController {
  ...
  setup(nextButtonCallback) {
    ...
    document
      .querySelectorAll('input[name="newInsuranceType"]')
      .forEach((radio) => {
    ...
    let setEndDateCheckbox = document.getElementById("setEndDateCheckbox");
    ...
    let effectiveEndDate = document.querySelector(
      ".new-policy-effective-end-date",
    );
    if (effectiveEndDate) {
      effectiveEndDate.value = window.top.oeFormatters.I18NDateFormat(
        this.__effectiveEndDate,
      );
    }
  ...
  updateCreateOption() {
    let createOption = document.querySelector(
      'input[name="createOption"]:checked',
    ).value;
  ...
  setCopyPolicyToFirstPolicy() {
    if (this.__isCopyPolicy) {
      let insurances = this.__insurancesByType[
        this.__selectedInsuranceType
  ...
  handleNextButtonPress(nextButtonCallback) {
    this.updateEffectiveEndDate()
      .then(() => {
        ...
        let newPolicy = this.getNewPolicy(
          data.copyPolicyId,
          data.isCopyPolicy,
          data.type,
          data.endDate,
      ...
      .catch((error) => {
        ...
        alert(
          xl("Failed to set effective end date for current policy.") +
            xl("Try again or contact your system administrator."),
        );
        console.error(error);
        this.__isSaving = false;
  ...
  getNewPolicy(copyPolicyId, isCopyPolicy, type) {
    ...
    return this.__insurancePolicyService.createInMemoryPolicy(
      type,
      copyPolicyId,
    );
  }
  ...
  toggleSetEffectiveEndDateRow(display) {
    let setEndDateRow = document.querySelectorAll(
      ".new-policy-set-end-date-row",
    );
    if (display) {
  ...
  shouldDisplayEffectiveEndDate() {
    let insurances =
      this.__insurancesByType[this.__selectedInsuranceType] || [];
  ...
  displayErrorMessage(errorCode) {
    ...
    let errorMessage = document.querySelector(
      ".new-policy-error-message-" + errorCode,
    );
    if (errorMessage) {
      errorMessage.classList.remove("d-none");
    } else {
      console.error(
        "Error message does not exist with class name: new-policy-error-message-" +
          errorCode,
      );
    }
  }
  ...
  updateEffectiveEndDate() {
    ...
    let insurance = this.__insurancePolicyService.getCurrentInsuranceForType(
      this.__selectedInsuranceType,
    );
  ...
  render() {
    ...
    document.getElementById(
      "new-insurance-type-" + this.__selectedInsuranceType,
    ).checked = true;
    document.getElementById("setEndDateCheckbox").checked =
      this.__setEffectiveEndDate;
    ...
    if (!this.__isCopyPolicy) {
      ...
      if (this.__copyPolicyId) {
        ...
        document.querySelector(".new-policy-copy-list").value =
          this.__copyPolicyId;
      }
    }
    ...
    let effectiveEndDateRows = document.querySelectorAll(
      ".new-policy-effective-end-date-row",
    );
    effectiveEndDateRows.forEach((r) => r.classList.add("d-none"));

    let policyCopyName = document.querySelector(
      ".effective-end-date-policy-label",
    );
    let mostRecentPolicy =
      this.__insurancePolicyService.getCurrentInsuranceForType(
        this.__selectedInsuranceType,
      );
  ...
  createCopyPolicyDropdown(newInsuranceType) {
    let insurancesByType = []
      .concat(
        Object.keys(this.__insurancesByType).map(
          (key) => this.__insurancesByType[key],
        ),
      )
      .flat();
    let insurances = insurancesByType.filter(
    ...
    if (insurances.length === 0) {
      ...
      insurances.forEach((insurance) => {
        ...
        option.innerText =
          this.capitalizeFirstLetter(insurance.type) +
          ": " +
          insurance.toString();
        if (
          ...
          option.innerText +=
            " - " + window.top.xl("End Date") + ": " + insurance.end_date;
        }
