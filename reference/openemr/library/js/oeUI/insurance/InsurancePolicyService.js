...
export class InsurancePolicyService {
  ...
  getCurrentPatientData() {
    ...
    let patientPromise = this.apiRequest(
      this.__apiURL + "patient/" + this.__puuid,
    );
    let employerDataPromise = this.apiRequest(
      this.__apiURL + "patient/" + this.__puuid + "/employer",
    );
    return Promise.all([patientPromise, employerDataPromise]).then(
      (resultData) => {
  ...
  getPatientInsuranceData(puuid, insuranceuuid) {
    return this.apiRequest(
      this.__apiURL + "patient/" + puuid + "/insurance/" + insuranceuuid,
    ).then((resultData) => {
  ...
  loadInsurancesByType() {
    return this.apiRequest(
      this.__apiURL + "patient/" + this.__puuid + "/insurance",
    ).then((resultData) => {
  ...
  storePolicyInMemory(policy) {
    ...
    let index = this.insurancesByType[policy.type].findIndex(
      (ins) => ins.id === policy.id,
    );
    if (index === -1) {
  ...
  swapInsurance(insurancePolicy, targetType) {
    ...
    return this.apiRequest(
      ...
      if (validationKeys.length > 0) {
        ...
        console.error(
          "Validation errors occurred. Error object",
        ...
        validationKeys.forEach((fieldName) => {
          ...
          validationFieldErrors.push(
            new ValidationFieldError(validationFieldName, validationErrors),
          );
        });
        throw new ValidationError(
          "Failed to save insurance policy due to validation errors",
      ...
      if (resultData.data.target) {
        let oldTargetInsurance = this.findInsuranceByUuid(
          resultData.data.target.uuid,
        );
      ...
      .then((resultData) => {
        ...
        if (validationKeys.length > 0) {
          ...
          validationKeys.forEach((fieldName) => {
            let validationErrors = resultData.validationErrors[fieldName];
            validationFieldErrors.push(
              new ValidationFieldError(fieldName, validationErrors),
            );
          });
          throw new ValidationError(
            "Failed to save insurance policy due to validation errors",
        ...
        if (resultData.data) {
          ...
          throw new Error(
            "Failed to save insurance policy as no data came back",
          );
        }
      });
  }
  ...
  #replacePolicyInMemory(oldPolicy, newPolicy) {
    ...
    let index = this.insurancesByType[oldPolicy.type].findIndex(
      (ins) => ins.id === oldPolicy.id,
    );
    if (index !== -1) {
