...
function moveOptions_11(theSelFrom, theSelTo) {
  if (!oeCustomTemplateEditor) {
    console.error(
      "CKEDITOR instance not found. Verify initAjaxFunctionWritersWithEditor was called",
    );
    return;
  }
...
function getCallingDocumentEditorContent(id, ccFlag) {
  ...
  if (ccFlag) {
    ...
    if (ccFlag === "id") {
      ...
      // must be name attr.
      val = window.opener.document.querySelector(
        "textarea[name=" + id + "]",
      ).value;
      if (val === null) {
        val = window.opener.document.querySelector(
...
function edit(id, ccFlag = "") {
  document.getElementById("textarea1").value = getCallingDocumentEditorContent(
    id,
    ccFlag,
  );
}
...
function SelectToSave(textara, ccFlag = "", contentToSave = "") {
  ...
  if (ccFlag) {
    ...
    if (
      mainForm.getElementById(textara + "_optionTD") &&
      document.getElementById("options")
    )
      mainForm.getElementById(textara + "_optionTD").innerHTML =
        document.getElementById("options").innerHTML;
    if (mainForm.getElementById(textara)) {
      mainForm.getElementById(textara).value = textAreaContent;
      if (document.getElementById("options")) {
        mainForm.getElementById(textara).value +=
          "|*|*|*|" + document.getElementById("options").innerHTML;
      }
    }
  }
