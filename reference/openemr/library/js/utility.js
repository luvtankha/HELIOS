...
function xl(string) {
  ...
  if (top.i18next && typeof top.i18next.t == "function") {
    ...
    console.log(
      "xl function is unable to translate since can not find the i18next.t function",
    );
    return string;
  }
}
...
function initInteractors(dragContext = document, resizeContext = "") {
  ...
  // init
  interact(".drag-action", { context: dragContext })
    .draggable({
      enabled: true,
      inertia: true,
      modifiers: [
  ...
  interact(".resize-action", { context: resizeContext })
    .resizable({
      enabled: true,
...
function checkPasswordStrength(inputElement) {
  ...
  if (pwd.length < 6) {
    document.getElementById("password_strength_meter").style.backgroundColor =
      "#ff6666";
    document.getElementById("password_strength_text").innerText =
      xl("Very Weak");
  } else {
    if (
      pwd.match(number) &&
...
if (typeof asyncAlertMsg !== "function") {
  ...
  function asyncAlertMsg(message, timer = 5000, type = "danger", size = "") {
    ...
    const sizeStyle =
      size === "lg" ? "left:25%;width:50%;" : "left:35%;width:30%;";
    const containerStyle =
      "position:fixed;top:25%;" + sizeStyle + "bottom:0;z-index:9999;";
  ...
  oeSMART.initLaunch = function (webroot, csrfToken) {
    ...
    for (let launch of smartLaunchers) {
      launch.addEventListener("click", function (evt) {
        ...
        dlgopen(url, "_blank", "modal-full", height, "", title, {
          allowExternal: true,
        });
      });
    }
    ...
    for (let dsiHelp of dsiHelpNodes) {
      dsiHelp.addEventListener("click", function (evt) {
        ...
        if (typeof dsi != "string" || dsi == "") {
          console.error(
            "missing data-dsi-service-id parameter for .smart-launch-dsi-info",
          );
          return;
        }
