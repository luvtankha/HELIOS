  ...
  this.setup = function (list) {
    ...
    if (dwvOemr.oemrDocumentUrl) {
      ...
      saveButton.appendChild(
        document.createTextNode(dwv.i18n("Save Image Edits")),
      );
      ...
      saveButton.appendChild(
        document.createTextNode(dwv.i18n("Download Image Edits")),
      );
    ...
    if (dwvOemr.oemrDocumentUrl) {
      ...
      restoreButton.appendChild(
        document.createTextNode(dwv.i18n("Restore Image Edits")),
      );
      node = app.getElement("openData");
      ...
      restoreButton.onclick = function () {
        ...
        fetch(oemrStateSaveUrl, {
          ...
          .then((rtn) => {
            ...
            const file = new File([rtn], "state.json", {
...
dwvOemr.gui.setup = function () {
  ...
  if ($("#dwv").attr("src")) {
    dwv.utils.getUrlFromUri = function (uri) {
      var url = null;
      if (
        dwv.env.askModernizr("urlparser") &&
